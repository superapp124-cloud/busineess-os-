...
export const setupNativeBridge = () => {
  ...
  (window as any).ChatrWeb = {
    ...
    getState: () => ({
      ...
      isAuthenticated: !!localStorage.getItem('sb-auth-token') || !!localStorage.getItem('sb-nuuuqazaoaozgblmvkzn-auth-token') || !!localStorage.getItem('sb-sbayuqgomlflmxgicplz-auth-token'),
    }),
