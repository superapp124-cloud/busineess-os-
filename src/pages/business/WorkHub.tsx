    ...
    <div className="flex-1 bg-[#0a0a0c] h-full overflow-hidden flex font-sans text-white relative">
      ...
      <div className="w-72 bg-black/40 border-r border-white/5 flex flex-col backdrop-blur-xl z-10">
        <div className="p-8 pb-4 flex flex-col gap-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-white">Work Hub</h1>
          <p className="text-sm text-slate-400 font-medium">Business Operations</p>
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-8">
          <nav className="space-y-1.5">
            <p className="px-4 text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Workspace</p>
            {navItems.map(item => (
              <button 
                key={item.id}
              ...
              >
                ...
                {item.badge && (
                  <span className={`ml-auto text-xs px-2 py-0.5 rounded-full ${activeTab === item.id ? 'bg-violet-500/20 text-violet-300' : 'bg-white/10 text-slate-300'}`}>
                    {item.badge}
                  </span>
                )}
              </button>
          ...
          <nav className="space-y-1.5">
            <p className="px-4 text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Applications</p>
            {appItems.map(item => (
              <button 
                key={item.id}
                onClick={() => setActiveTab(item.id)}
      ...
      <div className="flex-1 flex flex-col relative z-10">
        <div className="h-20 border-b border-white/5 flex items-center px-10 bg-black/20 backdrop-blur-md sticky top-0">
          <h2 className="text-xl font-semibold capitalize text-white tracking-tight">
            {activeTab.replace('-', ' ')}
          </h2>
          ...
          <div className="ml-auto flex items-center gap-4">
            <div className="relative group">
              ...
              <input 
                ...
                className="bg-black/40 border border-white/10 rounded-full py-2 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/50 transition-all w-64"
              />
            </div>
            ...
            <button 
              ...
              className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:shadow-[0_0_20px_rgba(124,58,237,0.4)] transition-all flex items-center gap-2"
            >
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-mono font-medium text-slate-500 bg-black/50 px-2 py-1 rounded-md border border-white/5">{item.id}</span>
                    <span className="text-xs font-medium bg-blue-500/10 text-blue-400 px-2.5 py-1 rounded-md border border-blue-500/20">{item.definition_id.replace('def_', '').replace('_', ' ')}</span>
                    {item.priority === 'High' && (
                      <span className="text-xs font-medium bg-rose-500/10 text-rose-400 px-2.5 py-1 rounded-md border border-rose-500/20 flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" />
                        High Priority
                      </span>
                    )}
                  </div>
                  <h3 className="text-lg font-semibold text-slate-200 group-hover:text-white transition-colors">{item.title}</h3>
              ...
              {slideMode === 'create' ? (
                ...
                <div className="space-y-8">
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6">Request Metadata</h3>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-slate-400 text-sm">Request ID</span>
                        <span className="font-mono text-white text-sm bg-black/50 px-2 py-1 rounded border border-white/10">{selectedItem?.id}</span>
                      </div>
                      <div className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-slate-400 text-sm">Status</span>
                        <span className="text-violet-400 text-sm font-medium">{selectedItem?.status}</span>
                      </div>
                      <div className="flex justify-between items-center pb-4 border-b border-white/5">
                        <span className="text-slate-400 text-sm">Priority</span>
                        <span className="text-rose-400 text-sm font-medium">{selectedItem?.priority || 'Medium'}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-400 text-sm">Requested By</span>
                        <span className="text-white text-sm font-medium">{selectedItem?.author || selectedItem?.owner_id}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {canApprove() ? (
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 text-center">
                      <p className="text-sm text-blue-400">This request is waiting on approval from a Manager.</p>
                    </div>
                  )}
                </div>
              )}
