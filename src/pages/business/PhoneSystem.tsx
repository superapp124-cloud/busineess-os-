        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex bg-white dark:bg-black/20 p-1 rounded-lg border border-gray-200 dark:border-white/10">
            <button 
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'logs' ? 'bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-white/60 hover:text-gray-900 dark:hover:text-white'}`}
            >
              Call Logs
            </button>
            <button 
              onClick={() => setActiveTab('ivr')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'ivr' ? 'bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-white/60 hover:text-gray-900 dark:hover:text-white'}`}
            >
              IVR Routing
            </button>
          </div>
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card className="dark:bg-black/20 border-gray-200 dark:border-white/10">
                ...
                <CardContent><div className="text-2xl font-bold">{missedCalls}</div></CardContent>
              </Card>
              <Card className="dark:bg-black/20 border-gray-200 dark:border-white/10">
                <CardHeader className="pb-2 flex flex-row items-center justify-between">
                  <CardTitle className="text-sm font-medium text-gray-500">Voicemails</CardTitle>
                  <Voicemail className="w-4 h-4 text-purple-500" />
                </CardHeader>
                <CardContent><div className="text-2xl font-bold">{voicemails}</div></CardContent>
              </Card>
              <Card className="dark:bg-black/20 border-gray-200 dark:border-white/10">
                <CardHeader className="pb-2 flex flex-row items-center justify-between">
                ...
                <CardContent><div className="text-2xl font-bold">{formatDuration(avgDuration)}</div></CardContent>
              </Card>
            </div>
                      ...
                      <div key={call.id} className="flex items-center justify-between p-4 bg-white dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500 dark:text-white/60">Called {call.receiver_number} • {formatDistanceToNow(new Date(call.created_at), { addSuffix: true })}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="text-sm font-medium text-gray-700 dark:text-white/80">{formatDuration(call.duration_seconds)}</div>
                          {call.recording_url && (
