      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <CardContent className="space-y-4">
              {newKeyRaw && (
                <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl mb-4">
                  <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-400 mb-2">Save this key now! It will not be shown again.</p>
              ...
              {isLoading ? (
                <div className="text-sm text-gray-500 text-center py-4">Loading keys...</div>
              ) : apiKeys.length === 0 ? (
                <div className="text-sm text-gray-500 text-center py-4">No API keys generated yet.</div>
              ) : (
                apiKeys.map((key) => (
                  <div key={key.id} className="p-4 bg-gray-100 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm text-gray-500 font-mono mt-1">pk_live_************************</div>
                    </div>
          ...
          <Card className="dark:bg-black/20 border-gray-200 dark:border-white/10">
            ...
            <CardContent className="space-y-4">
              {isLoading ? (
                <div className="text-sm text-gray-500 text-center py-4">Loading webhooks...</div>
              ) : webhooks.length === 0 ? (
                <div className="text-sm text-gray-500 text-center py-4">No webhooks configured yet.</div>
              ) : (
                webhooks.map((webhook) => (
                  <div key={webhook.id} className="p-4 bg-gray-100 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                    ...
                    <div className="text-sm text-gray-500 dark:text-white/60 mb-2 truncate">
