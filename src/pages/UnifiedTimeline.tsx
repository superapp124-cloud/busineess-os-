    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
      ...
      <div className="bg-[#15151c] rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:border-indigo-500/30 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-3xl font-bold text-white">{stats.repliesNeeded}</h2>
          <div className="w-8 h-8 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400">
            <MessageSquare className="w-4 h-4" />
          </div>
        </div>
        <div>
          <p className="text-sm font-medium text-slate-300">Replies needed</p>
          <p className="text-xs text-indigo-400 mt-1">High priority</p>
        </div>
      </div>
      ...
      <div className="bg-[#15151c] rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:border-amber-500/30 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-3xl font-bold text-white">{stats.billsDue}</h2>
          <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-400">
            <FileText className="w-4 h-4" />
          </div>
        </div>
        <div>
          <p className="text-sm font-medium text-slate-300">Bills due</p>
          <p className="text-xs text-slate-500 mt-1">₹12,450 total</p>
        </div>
      </div>
      ...
      <div className="bg-[#15151c] rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:border-blue-500/30 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-3xl font-bold text-white">{stats.meetings}</h2>
          <div className="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400">
            <Calendar className="w-4 h-4" />
          </div>
        </div>
        <div>
          <p className="text-sm font-medium text-slate-300">Meetings today</p>
          <p className="text-xs text-blue-400 mt-1">Next in 45 min</p>
        </div>
      </div>
      ...
      <div className="bg-[#15151c] rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:border-emerald-500/30 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-3xl font-bold text-white">{stats.threatsDetected}</h2>
          <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400">
            <Shield className="w-4 h-4" />
          </div>
        </div>
        <div>
          <p className="text-sm font-medium text-slate-300">Threats detected</p>
          <p className="text-xs text-slate-500 mt-1">You're all good</p>
        </div>
      </div>
      ...
      <div className="bg-[#15151c] rounded-2xl p-4 border border-white/5 flex flex-col justify-between group hover:border-purple-500/30 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-3xl font-bold text-white">{estMinutes} <span className="text-lg text-slate-400 font-normal">min</span></h2>
          <div className="w-8 h-8 rounded-full bg-purple-500/10 flex items-center justify-center text-purple-400">
            <Clock className="w-4 h-4" />
          </div>
        </div>
        <div>
          <p className="text-sm font-medium text-slate-300">Est. review time</p>
          <p className="text-xs text-slate-500 mt-1">Save 1.5 hours</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className={`rounded-2xl border transition-all duration-200 hover:bg-white/5 flex items-start gap-4 p-4 ${
      ...
      <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 border ${
        ...
        {event.source === 'mail' && event.sender.displayName?.includes('Amazon') ? (
           <span className="font-bold text-xl text-orange-400">a</span>
        ) : event.source === 'mail' && event.sender.displayName?.includes('HDFC') ? (
           <span className="font-bold text-xl text-blue-600">H</span>
        ) : event.source === 'sms' && event.sender.displayName?.includes('ICICI') ? (
           <span className="font-bold text-xl text-orange-500">i</span>
        ) : (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="flex items-center gap-2">
            <span className={`text-base font-semibold truncate ${isUnread ? 'text-white' : 'text-slate-300'}`}>
          ...
          <span className="text-xs font-medium text-slate-500">{relativeTime(event.timestamp)}</span>
        </div>

        {event.subject && (
          <p className={`text-sm mb-1 truncate ${isUnread ? 'text-slate-200 font-medium' : 'text-slate-400'}`}>
            {event.subject}
          </p>
        )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isThreat && (
            <span className="flex items-center gap-1 text-xs text-red-400 bg-red-500/10 px-2 py-1 rounded-md font-medium border border-red-500/20">
    ...
    <div className="flex gap-2 overflow-x-auto scrollbar-none">
      {filters.map(({ key, label }) => (
        <button
          ...
          className={`shrink-0 text-sm px-4 py-2 rounded-full font-medium transition-all ${
            active === key
      ...
      <div className="space-y-4">
         {[
            ...
            <div key={i} className="flex items-center gap-3 group">
               ...
               <button className="px-3 py-1.5 rounded-lg bg-indigo-500/10 text-indigo-400 text-xs font-semibold hover:bg-indigo-500/20 transition-colors opacity-0 group-hover:opacity-100">
                  Join
               </button>
            </div>
         ))}
   ...
   <div className="bg-[#15151c] rounded-2xl p-5 border border-white/5">
      ...
      <div className="space-y-5">
         <div className="flex items-start gap-3">
            ...
            <div>
               <h4 className="text-sm font-medium text-indigo-300">Airtel Prepaid plan expiring</h4>
               <p className="text-xs text-red-400 font-medium my-0.5">in 2 days</p>
               <p className="text-xs text-slate-500">Recharge ₹199 to continue</p>
            </div>
         </div>
         <div className="flex items-start gap-3">
            ...
            <div>
               <h4 className="text-sm font-medium text-white">Swiggy order arriving</h4>
               <p className="text-xs text-slate-400 font-medium my-0.5">in 35 mins</p>
               <p className="text-xs text-slate-500">Order #123456789012</p>
            </div>
         </div>
         <div className="flex items-start gap-3">
            ...
            <div>
               <h4 className="text-sm font-medium text-white">Flight to Delhi</h4>
               <p className="text-xs text-slate-400 font-medium my-0.5">Tomorrow, 7:45 AM</p>
               <p className="text-xs text-slate-500">AI237 • PNR: ABC123</p>
            </div>
         </div>
      </div>
   </div>
              ...
              <div className="lg:col-span-2">
                {isLoading ? (
                  <div className="flex flex-col items-center justify-center py-32 gap-4">
                    ...
                    <p className="text-sm text-slate-500 font-medium">Syncing intelligence…</p>
                  </div>
                ) : filtered.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="pt-4 pb-2 text-center">
                       <button className="text-sm font-semibold text-indigo-400 hover:text-indigo-300 flex items-center justify-center gap-1 mx-auto group">
