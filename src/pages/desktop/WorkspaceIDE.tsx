  ...
  return (
    <div className="flex flex-col h-full w-full py-4 px-3">
      <div className="flex items-center gap-2 px-2 mb-8 mt-2">
        ...
        <span className="font-semibold tracking-tight text-lg">CHATR</span>
      </div>

      <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 px-2">Workspaces</div>
      ...
      <div className="mt-auto space-y-1">
        <button className="w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
  ...
  if (context.type === 'empty') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-gradient-to-b from-background to-muted/5">
        ...
        <h1 className="text-4xl font-bold tracking-tight mb-3">Welcome to your Workspace</h1>
        <p className="text-muted-foreground text-lg mb-12 max-w-lg text-center leading-relaxed">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/30 px-5 py-2.5 rounded-full border border-border/50 backdrop-blur-sm">
          <Sparkles className="w-4 h-4 text-primary animate-pulse" />
          <span>Press <kbd className="px-2 py-1 bg-background rounded-md font-mono text-xs mx-1 border border-border/50 shadow-sm text-foreground">Cmd + K</kbd> to state your intent, or drop a file anywhere.</span>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl bg-card border border-border shadow-sm rounded-xl p-8 overflow-y-auto">
          {context.data?.vendor && (
            <div className="flex justify-between items-start mb-8 pb-4 border-b border-border/50">
              <div>
                <h1 className="text-2xl font-semibold mb-1">Entity Details</h1>
                <p className="text-muted-foreground">{context.data.vendor}</p>
              </div>
              {context.data?.amount && (
                <div className="text-right">
                  <p className="text-sm text-muted-foreground mb-1">Amount</p>
                  <p className="text-2xl font-semibold text-primary">{context.data.amount}</p>
                </div>
              )}
            </div>
          )}
