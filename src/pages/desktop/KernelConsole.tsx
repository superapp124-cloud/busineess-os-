...
export default function KernelConsole() {
  ...
  if (error) {
    return (
      <div className="p-8 text-red-500 font-mono">
        <h1 className="text-2xl mb-4 font-bold">KERNEL PANIC</h1>
        <p>{error}</p>
        <button 
  ...
  return (
    <div className="p-8 bg-zinc-950 text-emerald-500 font-mono min-h-screen">
      <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-widest">CHATR CORE <span className="text-emerald-500">v{health.version}</span></h1>
          <p className="text-zinc-400 text-sm mt-1">Codename: {health.codename} | Status: {health.core.toUpperCase()}</p>
        </div>
        <div className="text-right">
          <p className="text-zinc-500 text-xs uppercase tracking-widest">Uptime</p>
          <p className="text-lg text-white">{metrics.system?.uptimeSeconds ? `${metrics.system.uptimeSeconds}s` : 'N/A'}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-zinc-900 p-4 rounded border border-zinc-800">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wider">AI Provider</p>
          <p className="text-xl text-white capitalize">{health.provider}</p>
          <p className="text-xs text-zinc-400 mt-1">{health.providerOk ? '● Online' : '○ Offline'}</p>
        </div>
        <div className="bg-zinc-900 p-4 rounded border border-zinc-800">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wider">Active Model</p>
          <p className="text-xl text-white">{health.model}</p>
          <p className="text-xs text-zinc-400 mt-1">{health.latencyMs}ms ping</p>
        </div>
        <div className="bg-zinc-900 p-4 rounded border border-zinc-800">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wider">Memory Usage</p>
          <p className="text-xl text-white">{metrics.system?.memoryUsageMB ? `${metrics.system.memoryUsageMB} MB` : 'N/A'}</p>
        </div>
        <div className="bg-zinc-900 p-4 rounded border border-zinc-800">
          <p className="text-zinc-500 text-xs mb-1 uppercase tracking-wider">Total Requests</p>
          <p className="text-xl text-white">{metrics.requests || 0}</p>
          <p className="text-xs text-zinc-400 mt-1">Avg {metrics.avgLatencyMs || 0}ms / req</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Modules */}
        <div>
          <h2 className="text-lg font-bold text-white mb-4 border-b border-zinc-800 pb-2">MODULE REGISTRY</h2>
        ...
        {/* Lifecycle Inspector */}
        <div>
          <h2 className="text-lg font-bold text-white mb-4 border-b border-zinc-800 pb-2">LIFECYCLE INSPECTOR (Last 5)</h2>
            ...
            {metrics.inspector?.recentSpans?.slice(-5).reverse().map((span: any) => (
              <div key={span.requestId} className="mb-4 last:mb-0 border-b border-zinc-800 pb-4 last:border-0 last:pb-0">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs text-zinc-500 truncate mr-4">{span.requestId}</span>
