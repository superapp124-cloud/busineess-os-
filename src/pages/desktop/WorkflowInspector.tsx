    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className="text-xs font-mono text-slate-500 truncate">{record.workflowId.slice(0, 12)}…</span>
        </div>
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1 flex items-center justify-between bg-slate-800/50 rounded-lg px-3 py-2">
              <span className="text-sm text-slate-300">{stage.stageName}</span>
...
function EventStream({ record }: { record: WorkflowInspectorRecord }) {
  return (
    <div className="space-y-1.5 max-h-64 overflow-y-auto">
      ...
      {record.events.length === 0 && <p className="text-xs text-slate-600 italic">No events recorded yet.</p>}
    </div>
  );
}
      ...
      <div className="p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={cn('text-xs font-bold uppercase tracking-wider', statusColor)}>{record.status}</span>
          </div>
          <p className="text-[10px] font-mono text-slate-500 break-all">{record.workflowId}</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-bold text-white">{durationLabel(record.durationMs)}</p>
        ...
        {tab === 'policies' && (
          <div className="space-y-2">
            {record.policyEvaluations.length === 0
              ? <p className="text-xs text-slate-600 italic">No policy evaluations recorded.</p>
    ...
    <div className="flex flex-col h-full bg-slate-950 text-slate-200 font-sans">
      ...
      <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-white">Workflow Inspector</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="ml-2 flex items-center gap-1.5 text-xs text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-lg transition-colors border border-slate-700"
          >
        ...
        <div className="w-96 shrink-0 border-r border-slate-800 overflow-y-auto">
          {workflows.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-600 gap-3 py-16">
              ...
              <p className="text-sm">No workflows detected yet.</p>
        ...
        <div className="flex-1 overflow-hidden bg-slate-900">
          {selected ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-600 gap-3">
              ...
              <p className="text-sm">Select a workflow to inspect.</p>
            </div>
          )}
        </div>
      </div>
