...
export const Suggestions: React.FC = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-blue-600/20 to-violet-600/20 border border-blue-500/20 rounded-xl p-4 relative">
      ...
      <p className="text-sm font-semibold text-white/90 leading-snug mb-1">Recent {suggestion.entityType === 'meeting' ? 'Meeting' : 'Mention'}</p>
      <p className="text-xs text-white/60 mb-3">{suggestion.description}</p>
      
      <button className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-1.5 px-4 rounded-lg transition-colors">
        Open Context
      </button>
    </div>
  );
