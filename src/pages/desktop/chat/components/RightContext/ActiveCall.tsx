  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-white/90">Active Call</span>
      ...
      <div className="bg-white/[0.02] border border-emerald-500/20 rounded-xl p-4 space-y-4">
        <div>
          <h3 className="text-sm font-semibold text-white/90 truncate">{callRoomName || callRoomId}</h3>
          <p className="text-xs text-emerald-400/80">Connected {isVideo ? '(Video)' : '(Audio)'}</p>
        </div>
