...
import { CheckCheck, Smile, Reply, Forward, Sparkles, MoreVertical, FileText, Download } from 'lucide-react';
              ...
              if (isImage) {
                return (
                  <div key={i} className="relative rounded-lg overflow-hidden border border-white/10 group/img">
                    ...
                    <button onClick={() => onFullscreenImage(att.url)} className="absolute inset-0 w-full h-full bg-black/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center text-white text-xs font-semibold backdrop-blur-sm">View Full</button>
                  </div>
                );
              }
              return (
