...
export const AIInsights: React.FC = () => {
  ...
  const { chartData, productivity, onTrack, needAttention } = useMemo(() => {
    ...
    // Mock chart data with a slightly dynamic trend based on completion %
    const base = total > 0 ? (completed / total) * 100 : 50;
    const chart = Array.from({ length: 7 }).map((_, i) => ({
      value: Math.min(100, Math.max(20, base + (Math.random() * 40 - 20)))
    }));
    ...
    <div className="bg-gradient-to-br from-violet-900/40 to-indigo-900/20 border border-violet-500/20 rounded-2xl p-4 relative overflow-hidden group flex-1 flex flex-col h-full min-h-[150px]">
      <div className="relative z-10 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-bold text-white/90">AI Insights</h2>
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4 min-h-[120px]">
            ...
            <p className="text-sm font-semibold text-white/90">AI Intelligence</p>
            <p className="text-xs text-white/50 mt-2">Start collaborating and CHATR will analyze your team's productivity and automatically highlight critical items here.</p>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mt-auto">
              <div>
                <p className="text-2xl font-bold text-white">{productivity}</p>
              ...
              <div>
                <p className="text-2xl font-bold text-white">{needAttention}</p>
