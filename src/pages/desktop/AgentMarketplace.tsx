      ...
      <div className="max-w-5xl mx-auto w-full p-8 space-y-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
          {['All Agents', 'HR & Recruitment', 'Engineering', 'Legal', 'Sales', 'Healthcare', 'Finance'].map(cat => (
            <button key={cat} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${cat === 'All Agents' ? 'bg-slate-800 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              {cat}
            </button>
          ))}
        </div>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium mt-0.5">By {agent.creator}</p>
                      </div>
                      <span className="text-xs font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md capitalize">
                        {agent.price}
                      </span>
                    </div>
