  ...
  const Toggle = ({ label, desc, value, onChange }: any) => (
    <div className="flex items-center justify-between py-3">
      <div>
        ...
        {desc && <p className={cn('text-xs', labelColor)}>{desc}</p>}
      </div>
  ...
  return (
    <div className={cn('flex-1 overflow-y-auto p-8', bg)}>
      <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="mb-8">
          <h1 className={cn('text-3xl font-black tracking-tight', headingColor)}>Appearance</h1>
          ...
          <div className="grid grid-cols-3 gap-3">
            {THEMES.map(t => (
              <button key={t.id} onClick={() => saveTheme(t.id)}
                ...
                <span className={cn('text-xs font-semibold', themeMode === t.id ? 'text-indigo-400' : textColor)}>{t.label}</span>
