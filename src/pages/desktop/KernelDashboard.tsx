    ...
    <div className="flex h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="w-1/3 border-r border-white/10 flex flex-col">
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-white/50 bg-white/5 px-2 py-1 rounded">v1.0.0</span>
        </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          ...
          <div className="space-y-3">
            {liveCommitments.length === 0 && (
              <div className="text-white/40 text-sm text-center py-10">No active commitments</div>
            )}
            {liveCommitments.map(c => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="font-medium text-sm">{c.title || c.capability}</span>
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-white/10 text-white/70">
                    {c.status}
                  </span>
                </div>
                <div className="text-xs font-mono text-white/40">{c.capability}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="p-6 border-b border-white/10 bg-[#161616]">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-2xl font-bold">{latestCommitment.title || 'Untitled'}</h2>
            <p className="font-mono text-sm text-white/50 mt-1">{latestCommitment.id}</p>
          </div>
          <div className="flex gap-2">
            <span className="px-3 py-1 rounded bg-blue-500/20 text-blue-400 font-mono text-sm border border-blue-500/20">
              {latestCommitment.capability}
            </span>
            <span className="px-3 py-1 rounded bg-emerald-500/20 text-emerald-400 font-mono text-sm border border-emerald-500/20 uppercase tracking-widest">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="relative pl-4 border-l border-white/10 space-y-6">
            {timeline.slice().reverse().map((evt, idx) => (
              <div key={evt.id} className="relative">
                ...
                <div className="text-xs font-mono text-white/40 mb-1 flex justify-between">
                  <span>{new Date(evt.timestamp).toISOString()}</span>
                  <span>{evt.source}</span>
                </div>
                <div className="bg-white/5 border border-white/10 p-3 rounded-md font-mono text-sm">
        ...
        <section>
          ...
          <pre className="bg-black/50 p-4 rounded-lg border border-white/10 text-xs font-mono text-white/70 overflow-x-auto">
