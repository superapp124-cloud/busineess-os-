  ...
  const VisibilitySelect = ({ label, value, icon: Icon, onChange }: any) => (
    <div className="flex items-center justify-between py-3 border-b border-border/50 last:border-0">
      ...
      <select
        ...
        className={cn('text-xs font-medium px-3 py-1.5 rounded-lg border outline-none cursor-pointer', isDark ? 'bg-white/[0.05] border-white/[0.1] text-white' : 'bg-white border-slate-200 text-slate-700')}
      >
  ...
  const Toggle = ({ label, description, checked, onChange, icon: Icon }: any) => (
    <div className="flex items-center justify-between py-4 border-b border-border/50 last:border-0">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h4 className={cn('text-sm font-medium', textColor)}>{label}</h4>
          <p className={cn('text-xs mt-0.5 max-w-[240px]', labelColor)}>{description}</p>
        </div>
      </div>
      <button
  ...
  return (
    <div className={cn('flex-1 flex flex-col h-full overflow-hidden', bg)}>
      <div className={cn('h-14 border-b flex items-center px-4 shrink-0', cardBg)}>
        ...
        <h1 className={cn('text-sm font-semibold', headingColor)}>Privacy & Security</h1>
      </div>
        ...
        <div className="max-w-2xl mx-auto space-y-8 pb-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className={cn('text-lg font-bold', headingColor)}>Privacy Settings</h2>
                <p className={cn('text-sm', labelColor)}>Control who can see your information</p>
              </div>
            </div>
            ...
            <div className="grid gap-6">
              <div className={cn('p-2 rounded-2xl border', cardBg)}>
                <div className="px-4 py-2 border-b border-border/50">
                  <h3 className={cn('text-xs font-bold uppercase tracking-wider', labelColor)}>Visibility</h3>
                </div>
                <div className="px-4">
                  <VisibilitySelect
                    icon={Eye}
              ...
              <div className={cn('p-2 rounded-2xl border', cardBg)}>
                <div className="px-4 py-2 border-b border-border/50">
                  <h3 className={cn('text-xs font-bold uppercase tracking-wider', labelColor)}>Activity</h3>
                </div>
                <div className="px-4">
                  <Toggle
                    icon={CheckCircle}
