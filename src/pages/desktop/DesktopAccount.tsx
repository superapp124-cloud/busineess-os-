  ...
  return (
    <div className={cn('flex-1 overflow-y-auto p-8', bg)}>
      <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="mb-8">
          <h1 className={cn('text-3xl font-black tracking-tight', headingColor)}>Account</h1>
          ...
          {changingPassword ? (
            <div className="space-y-3">
              {[
                ...
                <div key={f.key}>
                  <label className={cn('text-xs font-semibold mb-1 block', labelColor)}>{f.label}</label>
                  <input
                    type="password"
                    value={(passwordForm as any)[f.key]}
                    onChange={e => setPasswordForm(p => ({ ...p, [f.key]: e.target.value }))}
                    className={cn('w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:border-indigo-500 transition-colors', inputBg)}
                    placeholder="••••••••"
                  />
                </div>
              ))}
              <div className="flex gap-2 mt-4">
                <button onClick={handlePasswordChange} disabled={saving}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-indigo-500 hover:bg-indigo-600 transition-colors disabled:opacity-50">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                  {saving ? 'Saving…' : 'Update Password'}
                </button>
                <button onClick={() => { setChangingPassword(false); setPasswordForm({ current: '', new: '', confirm: '' }); }}
                  className={cn('px-4 py-2 rounded-xl text-sm font-semibold transition-colors', isDark ? 'text-white/60 hover:bg-white/[0.06]' : 'text-slate-500 hover:bg-slate-100')}>
                  Cancel
                </button>
              </div>
            </div>
          ) : (
            <p className={cn('text-sm', labelColor)}>Last changed: Never • Use a strong, unique password</p>
          )}
        </div>
        ...
        <div className={cn('rounded-2xl border p-6', cardBg)}>
          ...
          <div className={cn('p-3 rounded-xl flex items-center justify-between', isDark ? 'bg-white/[0.04]' : 'bg-slate-50')}>
            <div>
              <p className={cn('text-sm font-medium', textColor)}>This device</p>
              <p className={cn('text-xs', labelColor)}>Browser • Current session</p>
            </div>
            <span className="text-xs text-emerald-400 font-semibold bg-emerald-400/10 px-2 py-1 rounded-full">Active</span>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-red-500/20 p-6 bg-red-500/5">
          ...
          <p className={cn('text-sm mb-4', labelColor)}>Permanently delete your account and all associated data. This cannot be undone.</p>
          <button onClick={handleDeleteAccount}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-red-400 border border-red-400/30 hover:bg-red-400/10 transition-colors">
