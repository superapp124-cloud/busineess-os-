      ...
      <div className={cn('flex items-center justify-between px-6 py-4 border-b', isDark ? 'border-white/[0.06]' : 'border-slate-200')}>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className={cn('text-lg font-bold min-w-[160px] text-center', headingColor)}>
              {MONTHS[month]} {year}
            </h2>
          ...
          <button onClick={() => setCurrentDate(new Date())}
            className={cn('text-xs px-3 py-1.5 rounded-lg font-medium transition-colors', isDark ? 'bg-white/[0.06] text-white/70 hover:bg-white/[0.1]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}>
            Today
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => calendarService.initiateOAuth('google')}
            className={cn('flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium transition-colors', isDark ? 'bg-white/[0.06] text-white/70 hover:bg-white/[0.1]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')}>
            <Link className="w-3.5 h-3.5" /> Connect Google
          </button>

          <button onClick={() => setShowNewEvent(true)}
            className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-lg font-semibold text-white transition-all hover:opacity-90"
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-7 mb-2">
            {DAYS.map(d => (
              <div key={d} className={cn('text-center text-xs font-semibold py-2', labelColor)}>{d}</div>
            ))}
          </div>
          {/* Cells */}
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} className={cn('min-h-[90px] border-b border-r p-1.5 transition-colors cursor-pointer', isDark ? 'border-white/[0.04]' : 'border-slate-100', cellBg)}>
                  <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold mb-1', isToday ? 'bg-indigo-500 text-white' : headingColor)}>
                    {day}
                  </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {events.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className={cn('text-sm', labelColor)}>No events this month</p>
              <button onClick={() => setShowNewEvent(true)}
                className="mt-2 px-4 py-2 rounded-lg text-sm font-medium text-indigo-400 border border-indigo-400/30 hover:bg-indigo-400/10 transition-colors">
              ...
              {events.sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime()).map(e => (
                <div key={e.id} className={cn('rounded-xl border p-4 flex items-start gap-3', cardBg)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={cn('text-xs mt-0.5', labelColor)}>
                      {new Date(e.start).toLocaleString()} {e.end && e.end !== e.start && `→ ${new Date(e.end).toLocaleTimeString()}`}
                    </p>
                    {e.location && <p className={cn('text-xs mt-1 flex items-center gap-1', labelColor)}><MapPin className="w-3 h-3" />{e.location}</p>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {showNewEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
          ...
          <div className={cn('w-full max-w-md rounded-2xl border p-6 space-y-4', cardBg)}>
            <h3 className={cn('text-lg font-bold', headingColor)}>New Event</h3>
            <input type="text" placeholder="Event title" value={newEvent.title}
              onChange={e => setNewEvent(p => ({ ...p, title: e.target.value }))}
              className={cn('w-full px-4 py-2.5 rounded-xl border text-sm outline-none focus:border-indigo-500', isDark ? 'bg-white/[0.05] border-white/[0.1] text-white placeholder:text-white/30' : 'bg-white border-slate-200')} />
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={cn('text-xs font-semibold block mb-1', labelColor)}>Start</label>
                <input type="datetime-local" value={newEvent.start}
                  onChange={e => setNewEvent(p => ({ ...p, start: e.target.value }))}
                  className={cn('w-full px-3 py-2 rounded-xl border text-sm outline-none focus:border-indigo-500', isDark ? 'bg-white/[0.05] border-white/[0.1] text-white' : 'bg-white border-slate-200')} />
              </div>
              <div>
                <label className={cn('text-xs font-semibold block mb-1', labelColor)}>End</label>
            ...
            {/* Color picker */}
            <div>
              <label className={cn('text-xs font-semibold block mb-2', labelColor)}>Color</label>
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleCreateEvent} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-500 hover:bg-indigo-600 transition-colors disabled:opacity-50">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                {saving ? 'Creating…' : 'Create Event'}
              </button>
              <button onClick={() => setShowNewEvent(false)}
                className={cn('px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors', isDark ? 'text-white/60 hover:bg-white/[0.06]' : 'text-slate-500 hover:bg-slate-100')}>
                Cancel
              </button>
            </div>
          </div>
