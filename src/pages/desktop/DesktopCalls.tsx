...
const DesktopCalls: React.FC = () => {
 ...
 const [showCallConnectModal, setShowCallConnectModal] = useState(false);
 ...
 {showCallConnectModal && (
   <StartCallConnectionModal
     isOpen={showCallConnectModal}
     onClose={() => setShowCallConnectModal(false)}
     goal={sessionGoal}
     onStart={(target) => {
       setDialInput(target);
       startCall(target, true);
       setShowCallConnectModal(false);
     }}
   />
 )}
...
/* ─── Start Call & Platform Connection Modal ─────────────────── */
const CALL_CONNECTIONS_SEED = [
  { id: 'usr-arshid-wani', name: 'Arshid Wani', handle: '@arshid_wani', role: 'Co-Founder & CTO', isOnPlatform: true, isOnline: true },
  { id: 'usr-sanobar-wani', name: 'Sanobar Wani', handle: '@sanobar_wani', role: 'Executive Operations', isOnPlatform: true, isOnline: true },
  { id: 'candidate_java_847', name: 'Rajesh Kumar', handle: '@rajesh_kumar', role: 'Senior Java Candidate', isOnPlatform: true, isOnline: true },
  { id: 'usr-vishal-sharma', name: 'Vishal Sharma', handle: '@vishal_sharma', role: 'Engineering Lead', isOnPlatform: true, isOnline: true },
  { id: 'usr-gaurav-verma', name: 'Gaurav Verma', handle: '@gaurav_verma', role: 'Product Manager', isOnPlatform: true, isOnline: false },
  { id: 'usr-priya-sharma', name: 'Priya Sharma', handle: '@priya_sharma', role: 'Lead Recruiter', isOnPlatform: true, isOnline: true },
  { id: 'usr-pooja-sharma', name: 'Pooja Sharma', handle: '@pooja_sharma', role: 'Talent Acquisition', isOnPlatform: true, isOnline: true },
  { id: 'usr-amit-varma', name: 'Amit Varma', handle: '@amit_varma', role: 'DevOps Specialist', isOnPlatform: true, isOnline: false },
];

function StartCallConnectionModal({
  isOpen,
  onClose,
  goal,
  onStart,
  ...
  useEffect(() => {
    supabase.from('profiles').select('id, username, full_name, avatar_url, phone_number').limit(30).then(({ data }) => {
      if (data && Array.isArray(data)) {
        setDbContacts(data.map((p: any) => ({
          id: p.id,
  ...
  const allConnections = React.useMemo(() => {
    const map = new Map();
    [...CALL_CONNECTIONS_SEED, ...dbContacts].forEach(c => map.set(c.id, c));
  ...
  const handleWhatsAppInvite = (targetInput: string) => {
    const cleanNum = targetInput.replace(/[^0-9]/g, '');
    const waUrl = `https://wa.me/${cleanNum || '919876543210'}?text=${encodeURIComponent('Hi! Join me on CHATR OS for a live voice/video call: https://chatrchat.in/desktop/calls')}`;
    window.open(waUrl, '_blank');
    toast.success(`WhatsApp invitation link generated for ${targetInput}!`);
  };

  const handleEmailInvite = (targetInput: string) => {
    const mailto = `mailto:${targetInput.includes('@') ? targetInput : 'invite@chatrchat.in'}?subject=${encodeURIComponent('Invitation to CHATR Live Call')}&body=${encodeURIComponent('Hi, join my live session on CHATR OS: https://chatrchat.in/desktop/calls')}`;
    window.open(mailto, '_blank');
    toast.success(`Email invitation sent to ${targetInput}!`);
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-[#0f1422] border border-purple-500/30 w-full max-w-lg rounded-3xl p-6 shadow-2xl space-y-4 text-white">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div>
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              ...
              {goal ? `Start Session: ${goal.toUpperCase()}` : 'Start Call / Connect User'}
            </h2>
            <p className="text-xs text-zinc-400 mt-0.5">Select a connection to call or invite any user by phone or email.</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-white/10 text-zinc-400 hover:text-white cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-2">
          <div className="relative">
            ...
            <input
              type="text"
              autoFocus
              placeholder="Search name or enter phone (+91...) or email..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-zinc-900 border border-white/15 focus:border-purple-500 rounded-2xl pl-10 pr-4 py-2.5 text-xs text-white placeholder:text-zinc-500 focus:outline-none"
            />
          </div>

          {/* Platform Status Detection Badge */}
          {query && (
            <div className="p-3 bg-black/40 rounded-2xl border border-white/10 flex items-center justify-between text-xs">
              {isPlatformUser ? (
                <div className="flex items-center gap-2 text-emerald-400 font-bold">
                  <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  <span>User Registered on CHATR Platform</span>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-amber-300 font-bold">
                  <div className="w-2 h-2 rounded-full bg-amber-400" />
                  <span>Not Registered on CHATR Yet</span>
                </div>
              )}
              ...
              {!isPlatformUser && (
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => handleWhatsAppInvite(search)}
                    className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-[10px] rounded-xl flex items-center gap-1 transition-colors cursor-pointer"
                  >
                    WhatsApp Invite
                  </button>
                  <button
                    onClick={() => handleEmailInvite(search)}
                    className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[10px] rounded-xl flex items-center gap-1 transition-colors cursor-pointer"
                  >
                    Email Invite
                  </button>
                </div>
        ...
        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
          <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 px-1">
            Connections & Workspace Directory ({filtered.length})
          </div>

          {filtered.map(c => (
            <div key={c.id} className="p-3 bg-zinc-900/60 hover:bg-zinc-800/80 border border-white/5 hover:border-purple-500/30 rounded-2xl flex items-center justify-between transition-all">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-purple-900/50 border border-purple-500/30 flex items-center justify-center font-bold text-purple-200 text-xs">
                  {c.name[0]}
                </div>
                <div>
                  <div className="font-bold text-xs text-white flex items-center gap-1.5">
                    <span>{c.name}</span>
                    <span className="text-[9px] px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 font-normal">Online</span>
                  </div>
                  <div className="text-[10px] text-zinc-400">{c.handle} · {c.role}</div>
                </div>
              </div>
