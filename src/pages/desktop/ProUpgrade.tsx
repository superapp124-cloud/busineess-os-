    ...
    <div className={cn("flex flex-col h-full w-full overflow-hidden", isDark ? "bg-[#0B0F19] text-white" : "bg-slate-50 text-slate-900")}>
      <ScrollArea className="flex-1">
        <div className="max-w-5xl mx-auto px-8 py-16">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 text-sm font-medium text-purple-600 dark:text-purple-400 mb-2">
            ...
            <p className={cn("text-lg max-w-2xl mx-auto font-medium", isDark ? "text-slate-400" : "text-slate-600")}>
          ...
          >
            <div className={cn("inline-flex items-center p-1.5 rounded-full border shadow-sm", isDark ? "bg-[#13141C] border-white/10" : "bg-white border-slate-200")}>
              <button 
                ...
                className={cn("px-6 py-2.5 rounded-full text-sm font-semibold transition-all", billingCycle === 'monthly' ? (isDark ? "bg-slate-800 text-white shadow" : "bg-slate-100 text-slate-900 shadow-sm") : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300")}
              >
                Monthly
              </button>
              <button 
                onClick={() => setBillingCycle('annual')}
                className={cn("px-6 py-2.5 rounded-full text-sm font-semibold transition-all flex items-center gap-2", billingCycle === 'annual' ? (isDark ? "bg-slate-800 text-white shadow" : "bg-slate-100 text-slate-900 shadow-sm") : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300")}
              >
              ...
              {features.map((feature, i) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className={cn("text-sm leading-relaxed", isDark ? "text-white/70" : "text-slate-700/80")}>
                      {feature.description}
                    </p>
                  </div>
                </motion.div>
              ...
              <div className={cn(
                ...
                <div className="mb-8">
                  ...
                  <div className="flex items-end gap-2 mb-2">
                    ...
                    <span className={cn("text-lg font-medium mb-1", isDark ? "text-slate-400" : "text-slate-500")}>/mo</span>
                  </div>
                  {billingCycle === 'annual' && (
                    <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">Billed $180 annually</p>
                  )}
                </div>
                ...
                <Button className="w-full rounded-2xl h-14 text-base font-bold bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white border-0 shadow-lg shadow-purple-500/25 transition-all hover:scale-[1.02]">
                  Unlock Pro Now
