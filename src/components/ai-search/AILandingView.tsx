...
export function AILandingView({ onSearch }: AILandingViewProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[85vh] w-full px-4 text-center">
      ...
      {/* Subtitle */}
      <p className="text-[#5a677f] font-semibold tracking-[0.2em] text-xs sm:text-sm mb-12 uppercase">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-4xl mb-6">
        <div className="relative flex items-center bg-[#111827] border border-[#1f2937] rounded-2xl transition-all hover:border-[#374151] focus-within:border-[#4f46e5] focus-within:ring-1 focus-within:ring-[#4f46e5] shadow-2xl overflow-hidden p-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none text-white placeholder-gray-500 text-lg sm:text-xl py-3 w-full"
            autoFocus
          />
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mb-16">
        <button className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 transition-colors text-sm">
        ...
        <button className="flex items-center gap-2 px-6 py-2 rounded-xl border border-[#818cf8] text-white bg-[#312e81]/30 transition-colors text-sm font-medium">
      ...
      <div className="w-full max-w-4xl">
        <h3 className="text-[#4b5563] text-xs font-semibold tracking-widest uppercase mb-4">Try Asking</h3>
        <div className="flex flex-wrap justify-center gap-3">
          {suggestions.map((suggestion, i) => (
            <button
              key={i}
              onClick={() => onSearch(suggestion)}
              className="px-5 py-2.5 rounded-xl border border-white/5 bg-[#111827]/50 text-gray-300 hover:text-white hover:bg-[#1f2937] hover:border-white/10 transition-colors text-sm"
            >
              {suggestion}
            </button>
          ))}
      ...
      {/* Footer Details */}
      <div className="fixed bottom-6 flex items-center gap-6 text-xs text-[#4b5563]">
