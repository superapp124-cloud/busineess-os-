...
export function SourceCardsPanel({ sources, isLoading }: SourceCardsPanelProps) {
  if (isLoading && sources.length === 0) {
    return (
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-gray-400 mb-2 uppercase tracking-wider">Live Web Sources</h3>
        {[1, 2, 3, 4].map(i => (
  ...
  const getTrustBadge = (url: string) => {
    if (url.includes(".gov.in") || url.includes("uidai") || url.includes("incometax")) {
      return <div className="flex items-center gap-1 text-xs text-green-400 bg-green-400/10 px-2 py-0.5 rounded"><Building2 className="w-3 h-3" /> Govt</div>;
    }
    if (url.includes("iplt20") || url.includes("bcci")) {
      return <div className="flex items-center gap-1 text-xs text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded"><ShieldCheck className="w-3 h-3" /> Official</div>;
    }
    if (url.includes("thehindu") || url.includes("indianexpress") || url.includes("moneycontrol")) {
      return <div className="flex items-center gap-1 text-xs text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded"><Newspaper className="w-3 h-3" /> News</div>;
    }
    return null;
  };
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                />
                <span className="text-xs text-gray-400 truncate">{getDomain(source.url)}</span>
              </div>
              <div className="flex items-center gap-2">
                {getTrustBadge(source.url)}
                <span className="text-xs text-indigo-400 bg-indigo-400/10 px-1.5 py-0.5 rounded font-mono">[{source.index}]</span>
              </div>
            </div>
            
            <h4 className="text-sm font-medium text-gray-200 mb-1 group-hover:text-indigo-300 transition-colors line-clamp-2">
              {source.title}
            </h4>
