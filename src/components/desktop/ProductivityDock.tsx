    ...
    <div ref={panelRef} className="fixed bottom-5 right-5 z-[8000] flex flex-col items-end gap-2">
      ...
      {activePanel === 'clipboard' && (
        <div className={cn(
          ...
          <div className={cn(
            'text-xs rounded-xl p-3 font-mono break-all max-h-32 overflow-auto leading-relaxed',
          ...
          {clipboardContent && clipboardContent.startsWith('http') && (
            <div className="mt-2 text-xs text-violet-400 bg-violet-500/10 px-3 py-1.5 rounded-lg font-medium">
      ...
      {activePanel === 'notes' && (
        <div className={cn(
          ...
          <div className="flex items-center justify-between mb-3">
            <span className={cn('text-sm font-semibold flex items-center gap-2', isDark ? 'text-white' : 'text-zinc-900')}>
          ...
          <textarea
            ...
            className={cn(
              'w-full h-32 text-sm bg-transparent outline-none resize-none leading-relaxed',
      ...
      {activePanel === 'translate' && (
        <div className={cn(
          ...
          <p className={cn('text-xs mb-3', isDark ? 'text-white/50' : 'text-zinc-500')}>
            Ask AI to translate your clipboard content:
          </p>
          <button
            onClick={() => { navigate('/desktop/canvas'); setActivePanel(null); }}
            className="w-full py-2 rounded-xl bg-emerald-600/20 text-emerald-400 text-sm font-semibold hover:bg-emerald-600/30 transition-colors border border-emerald-500/20"
          >
            Open AI Translator →
          </button>
        </div>
      ...
      {activePanel === 'voice' && (
        <div className={cn(
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className={cn('text-xs', isDark ? 'text-white/40' : 'text-zinc-400')}>Tap to record voice note</p>
          </div>
        </div>
      )}
