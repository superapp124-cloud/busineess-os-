      ...
      <PopoverContent className="w-80 bg-[#11111a] border-white/10 text-white p-4 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] backdrop-blur-xl" align="start">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-bold text-sm">World Clock</h4>
          </div>
          <span className="text-xs font-medium text-white/50 bg-white/5 px-2 py-1 rounded-md">{timezones.length}/4 zones</span>
        </div>
        ...
        <div className="space-y-2 mb-4">
          {timezones.map(tz => (
            <div key={tz} className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5 group hover:bg-white/10 transition-colors">
              <div>
                <div className="text-sm font-bold text-white/90">{getLabel(tz)}</div>
                <div className="text-[11px] text-white/50 mt-0.5">{tz === 'local' ? 'Current Timezone' : tz.replace('_', ' ')}</div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-sm font-medium text-purple-200">{times[tz]}</div>
                {tz !== 'local' && (
        ...
        {timezones.length < 4 && (
          <div className="mt-2 pt-3 border-t border-white/5">
            <Select onValueChange={addTimezone}>
              <SelectTrigger className="w-full bg-white/5 hover:bg-white/10 border-white/10 text-sm h-11 rounded-xl transition-colors">
