      ...
      <div className="flex-1 flex flex-col h-full bg-background p-6">
        ...
        <div className="max-w-3xl flex-1 flex flex-col gap-6">
          ...
          <div className="flex-1 flex flex-col bg-card/50 p-6 rounded-2xl border border-border/50 relative">
            <label className="text-sm font-medium text-slate-400 mb-3 block">Template Content</label>
            <Textarea 
              placeholder="Use {name} or {variable} to insert dynamic fields..."
              className="flex-1 resize-none bg-background/50 border-border/50 font-mono text-sm"
            />
          </div>
        </div>
      </div>
    ...
    <div className="flex-1 flex flex-col h-full bg-background p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">Reusable content for broadcasts, chats, and AI.</p>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pr-4">
          {templates.filter(t => t.name.toLowerCase().includes(search.toLowerCase())).map(template => (
            ...
            >
              ...
              <h3 className="font-bold text-lg text-slate-200 mb-2">{template.name}</h3>
              <p className="text-sm text-slate-400 flex-1 font-mono bg-background/50 p-3 rounded-lg border border-border/30 line-clamp-4">
                {template.content}
              </p>
