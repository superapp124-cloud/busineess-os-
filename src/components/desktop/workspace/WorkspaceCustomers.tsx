    ...
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden relative">
      ...
      <div className="p-6 border-b border-border/50 bg-card/30 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold">{customer.name} Workspace</h2>
            <div className="flex items-center gap-2 mt-1">
              <Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500/20">
                {customer.segment}
              </Badge>
              <span className="text-xs text-slate-400">Last active 2 hrs ago</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 border-r border-border/50 flex flex-col">
          <ScrollArea className="flex-1 p-6">
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Unified Timeline</h3>
              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                  <div className="flex items-center justify-between space-x-2 mb-1">
                    ...
                    <time className="font-medium text-xs text-slate-500">Just now</time>
                  </div>
                  <div className="text-slate-500 dark:text-slate-400 text-sm">Customer replied to "Summer Sale Announcement".</div>
                </div>
              </div>
              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                  <div className="flex items-center justify-between space-x-2 mb-1">
                    ...
                    <time className="font-medium text-xs text-slate-500">2 hrs ago</time>
                  </div>
                  <div className="text-slate-500 dark:text-slate-400 text-sm">Targeted segment: Leads.</div>
                </div>
              </div>
            </div>
          </ScrollArea>
        ...
        <div className="w-80 bg-card/30 flex flex-col p-4 space-y-6">
          <div>
            ...
            <div className="p-3 bg-background rounded-lg border border-border/50 cursor-pointer hover:border-slate-600 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <p className="font-medium text-sm">Enterprise License</p>
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">AI Intelligence</h3>
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold">Customer Workspaces</h2>
          <p className="text-sm text-slate-400">Manage all entities and their unified history</p>
        </div>
      ...
      <div className="flex gap-2 mb-6">
        {['All', 'VIP', 'Leads', 'Pending Payments', 'Suppliers'].map(tag => (
          <Badge key={tag} variant="secondary" className="cursor-pointer hover:bg-slate-800 px-3 py-1 text-sm">{tag}</Badge>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pr-4">
          {customers.filter(c => c.name.toLowerCase().includes(search.toLowerCase())).map(customer => (
            ...
            >
              ...
              <h3 className="font-bold text-lg text-slate-200 mb-1 truncate">{customer.name}</h3>
              <div className="flex items-center justify-between mt-3">
                <Badge variant="outline" className="text-xs bg-slate-900/50">{customer.segment}</Badge>
