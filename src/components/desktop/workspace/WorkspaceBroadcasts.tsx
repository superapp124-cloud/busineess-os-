      ...
      <div className="flex-1 flex flex-col h-full bg-background p-6">
        ...
        <div className="max-w-3xl flex-1 flex flex-col gap-6">
          <div className="space-y-4 bg-card/50 p-6 rounded-2xl border border-border/50">
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Campaign Name</label>
              <Input 
                value={name}
            ...
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Target Audience</label>
                   ...
                   <ScrollArea className="flex-1">
                     <div className="space-y-3 pr-3">
                       {personalizedPreview.map((preview, i) => (
                         <div key={i} className="bg-background/80 p-3 rounded-lg text-sm border border-border/50">
                           <span className="text-xs font-semibold text-slate-400 block mb-1">To: {preview.name}</span>
                           {preview.content}
                         </div>
                       ))}
                     </div>
    ...
    <div className="flex-1 flex flex-col h-full bg-background p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">AI-powered outreach and campaigns</p>
        </div>
              ...
              <div key={broadcast.id} className="p-4 bg-card/50 border border-border/50 rounded-xl hover:bg-card transition-colors flex items-center justify-between group">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className="text-xs font-medium text-emerald-500 mb-1">Status: {broadcast.status.toUpperCase()}</p>
