...
export const DiagnosticsPanel: React.FC = () => {
  ...
  return (
    <div className="w-full h-full p-6 bg-slate-900 text-slate-100 font-mono text-sm overflow-y-auto custom-scrollbar flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between mb-6 pb-6 border-b border-slate-800">
        ...
        <div className="text-right flex items-center gap-6">
          <div className="text-center">
            <div className={`text-3xl font-bold ${healthScore > 90 ? 'text-emerald-400' : healthScore > 70 ? 'text-yellow-400' : 'text-rose-400'}`}>
              {healthScore}%
            </div>
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-slate-500 uppercase tracking-wider">ev/s</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2">
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
              ...
              <div className="space-y-2">
                {Object.entries(runtimeState.engineStatuses).map(([id, status]) => (
                  <div key={id} className="flex items-center justify-between bg-slate-900/50 p-2 px-3 rounded text-xs border border-slate-700/30">
          ...
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/30">
                <div className="text-slate-400 text-xs uppercase tracking-wider mb-1">Active Calls</div>
                <div className="text-2xl font-bold text-emerald-400">
                  {kernelAPI.hasEngine('CallEngine') ? (kernelAPI.getEngine<any>('CallEngine').metrics().activeSessions || 0) : 0}
                </div>
              </div>
              <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/30">
                <div className="text-slate-400 text-xs uppercase tracking-wider mb-1">Transcript Buffer</div>
              ...
              {Object.entries(flags).map(([id, enabled]) => (
                <div key={id} className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg border border-slate-700/30">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">Toggle Kernel vs Legacy implementations</div>
                  </div>
                  <button 
                    onClick={() => handleToggleFlag(id)}
                    className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase transition-colors ${enabled ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-slate-800 text-slate-400 border border-slate-600'}`}
                  >
        ...
        {activeTab === 'chaos' && (
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
            ...
            <p className="text-slate-400 text-xs mb-6">Inject deliberate failures to verify the RuntimeSupervisor's recovery and isolation mechanisms.</p>
            ...
            <div className="mt-8 border-t border-slate-700/50 pt-6">
              ...
              <p className="text-slate-400 text-xs mb-4">Run automated contract tests against the live Kernel API to ensure compatibility.</p>
              
              <button 
                onClick={handleRunTests}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded font-bold text-xs"
              >
              ...
              {contractResults && (
                <div className="mt-4 p-4 bg-slate-900/50 rounded-lg border border-slate-700/30">
                  ...
                  {contractResults.errors.length > 0 && (
                    <div className="text-xs text-rose-400 mt-2 space-y-1">
