...
const QUICK_ACTIONS: QuickAction[] = [
  { icon: MessageSquare, label: 'New Chat',     color: 'text-emerald-400', hoverColor: 'hover:bg-emerald-500/10', event: 'open-new-chat' },
  { icon: Phone,         label: 'Call',          color: 'text-blue-400',    hoverColor: 'hover:bg-blue-500/10',    event: 'open-new-call' },
  { icon: Video,         label: 'Video Meeting', color: 'text-violet-400',  hoverColor: 'hover:bg-violet-500/10', event: 'open-new-video' },
  { icon: Users,         label: 'New Group',     color: 'text-amber-400',   hoverColor: 'hover:bg-amber-500/10',  event: 'open-new-group' },
...
export const QuickActionsBar: React.FC<QuickActionsBarProps> = ({ isDark }) => {
  ...
  const handleAction = (action: QuickAction) => {
    if (action.path) {
      ...
      // Fallback navigation if we want to ensure they are on the chat page to see the modal
  ...
  return (
    <div className={cn(
      ...
      {/* Label */}
      <span className={cn('text-[10px] font-semibold uppercase tracking-widest mr-2 shrink-0', isDark ? 'text-white/50' : 'text-zinc-500')}>
        Quick Actions
      </span>
