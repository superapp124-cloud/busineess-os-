    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/8">
          ...
          />
          {query && (
            <button onClick={() => setQuery('')} className="text-white/30 hover:text-white/60 text-xs transition-colors">
              Clear
            </button>
          )}
        ...
        <ScrollArea className="max-h-[420px]">
            ...
            <div className="px-2 pt-3 pb-1">
              ...
              {recentSearches.map(r => (
                <button
                  key={r}
                  onClick={() => setQuery(r)}
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/5 text-sm text-white/50 hover:text-white/90 transition-colors"
                >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={cn('text-sm font-medium truncate', isSelected ? 'text-white' : 'text-white/80')}>
                        {item.label}
                      </p>
                      {item.description && (
                        <p className="text-xs text-white/35 truncate">{item.description}</p>
                      )}
                    </div>
                    {item.shortcut && (
          ...
          {allResults.length === 0 && !isSearching && (
            <div className="px-4 py-12 text-center">
              ...
              <p className="text-white/25 text-xs mt-1">Try "Call Rahul", "Ask AI", "Find Invoice"</p>
            </div>
          )}
        </ScrollArea>
