          ...
          >
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    {result.name}
                    {idx === 0 && <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full font-medium">🏆 Top Pick</span>}
                  </h3>
                  {result.badge && (
                    <span className="inline-block mt-1 text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full font-medium">
                      {result.badge}
                    </span>
                  )}
                </div>
                <div className="text-right">
                  <span className="text-xl font-bold text-white">{result.price}</span>
                </div>
              </div>
            ...
            <div className={`bg-gray-800/50 border-t border-gray-800 px-4 transition-all duration-300 ${isExpanded ? 'py-4 max-h-64' : 'max-h-0 py-0 opacity-0'} overflow-hidden`}>
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  <span className="text-sm font-semibold text-gray-200 block mb-1">Why this recommendation?</span>
            ...
            {!isExpanded && (
              <div className="bg-gray-800/30 py-2 text-center text-xs text-gray-500 flex items-center justify-center gap-1 hover:text-gray-300 transition-colors">
