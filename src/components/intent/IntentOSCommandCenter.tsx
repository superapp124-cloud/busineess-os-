...
function generateCustomWorkflow(query: string): IntentWorkflowData {
  const cleanQuery = query.trim() || "General Search & Information Retrieval";
  const slug = cleanQuery.toLowerCase().replace(/[^a-z0-9]/g, "-").slice(0, 30);
  return {
    id: "custom-" + slug,
    query: cleanQuery,
    intentName: "Information Retrieval & Discovery",
    categoryIcon: "tech",
    entities: [
      { label: "Query", value: cleanQuery.length > 20 ? cleanQuery.slice(0, 20) + "..." : cleanQuery, icon: "🔍", highlighted: true },
      { label: "Mode", value: "Deep Web Search", icon: "🌐" },
      { label: "Sources", value: "Verified Public Records", icon: "📑" },
    ],
    preferences: ["High accuracy", "Recent information", "Verified sources"],
    confidence: 90,
    executionPlan: [
      "Analyze search intent and extract key entities",
      "Query major search engines and knowledge graphs",
      "Cross-reference public databases and news archives",
      "Extract relevant snippets and summarize findings",
      "Compile verified information report",
    ],
    parallelWorkersCount: 12,
    liveResearch: [
      { name: "Google Knowledge Graph", domain: "google.com", latency: "0.22s", status: "done" },
      { name: "Wikipedia En", domain: "wikipedia.org", latency: "0.18s", status: "done" },
      { name: "LinkedIn Public Profiles", domain: "linkedin.com", latency: "0.35s", status: "done" },
      { name: "Bing Web Search", domain: "bing.com", latency: "0.27s", status: "done" },
      { name: "News Aggregator", domain: "news.google.com", latency: "0.29s", status: "done" },
    ],
    sourceQuality: [
      { domain: "google.com", score: 98, color: "#10B981" },
      { domain: "wikipedia.org", score: 95, color: "#10B981" },
      { domain: "linkedin.com", score: 92, color: "#10B981" },
      { domain: "bing.com", score: 94, color: "#10B981" },
    ],
    verifiedSourcesCount: 12,
    bestOptions: [
      {
        provider: "Primary Search Result",
        routeOrTitle: cleanQuery.length > 40 ? cleanQuery.slice(0, 40) + "..." : cleanQuery,
        subtext: "Top matched entity from public web sources",
        priceOrValue: "High Match",
        badge: "Top Result",
        isTop: true,
      },
      {
        provider: "Secondary Context",
        routeOrTitle: "Related professional and public records",
        subtext: "Aggregated from social and professional networks",
        priceOrValue: "Verified",
      },
      {
        provider: "News & Mentions",
        routeOrTitle: "Recent articles and public mentions",
        subtext: "Scanned across major news outlets",
        priceOrValue: "Latest",
      },
    ],
    totalOptionsCount: 5,
    actionCards: [
      { title: "View Full Profile", subtitle: "Open detailed aggregated report", icon: "📄", actionText: "View Details" },
      { title: "Set Alert", subtitle: "Notify on new mentions", icon: "🔔", actionText: "Set Alert" },
      { title: "Save to Knowledge Base", subtitle: "Index in local database", icon: "💾", actionText: "Save Info" },
      { title: "Share Findings", subtitle: "Export summary to PDF", icon: "📤", actionText: "Export" },
    ],
    featuredCard: {
      badge: "Verified Information Match",
      title: cleanQuery.length > 32 ? cleanQuery.slice(0, 32) + "..." : cleanQuery,
      subtitle: "Synthesized across major search engines and public databases",
      providerName: "Web Search Engine",
      providerLogoText: "🌐",
      mainPriceOrHighlight: "Match Found",
      priceSubtext: "Information gathered from multiple verified sources",
      buttonText: "Open Detailed Report",
      url: "https://www.google.com/search?q=" + encodeURIComponent(cleanQuery),
      whyChecklist: [
        "Cross-referenced across multiple independent search engines",
        "High confidence score based on entity matching",
        "Recent and historically consistent information",
      ],
      historyValues: [50, 60, 70, 75, 80, 85, 90, 92, 95, 98, 99, 100],
    },
    progressMetrics: {
      sources: 12,
      resultsProcessed: 145,
      trustedResults: 8,
      timeTaken: "1.2s",
    },
    smartSuggestions: [
      { label: "Refine search", subtext: "Add location or company", icon: "🔍" },
      { label: "Deep background check", subtext: "Search public records", icon: "📋" },
      { label: "Find images", subtext: "Search for related media", icon: "🖼️" },
    ],
    nextBestActions: [
      { title: "Download Summary", priceOrSubtext: "Save as PDF", icon: "📥", badge: "Recommended" },
      { title: "Email Report", priceOrSubtext: "Send findings to inbox", icon: "✉️" },
      { title: "Advanced Search", priceOrSubtext: "Use boolean operators", icon: "⚙️" },
    ],
  };
}
