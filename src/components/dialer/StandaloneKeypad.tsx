  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-[70vh] pb-24 px-8 smooth-mount">
      ...
      <div className="flex flex-col items-center justify-center h-32 w-full">
        <div className="text-4xl font-bold tracking-widest text-white mb-2 h-12">
          {number}
        </div>
        {number && (
          <button className="text-primary text-xs font-bold uppercase tracking-widest animate-in fade-in slide-in-from-top-2">
            Add Contact
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-x-8 gap-y-6">
        {keys.map((key) => (
          ...
          >
            <span className="text-3xl font-medium group-active:text-primary transition-colors">{key.num}</span>
