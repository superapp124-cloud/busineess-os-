      ...
      <SignalSection title="Community Signals" icon={Users} defaultOpen={true}>
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
            <span className="text-xs text-zinc-500">Spam Reports</span>
            <span className="font-bold text-red-500">{caller.communityReportCount}</span>
          </div>
          <div className="flex justify-between items-center bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
            <span className="text-xs text-zinc-500">Lookups in last 24h</span>
        ...
        {caller.webIdentity ? (
          <div className="space-y-4">
            <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
              ...
              <div className="text-xs text-zinc-500">{caller.webIdentity.category}</div>
            </div>
            <div className="flex justify-between items-center px-1">
              <span className="text-xs text-zinc-500">TRAI DND Status</span>
              <span className={cn(
                "text-xs font-bold",
          ...
          <div className="text-xs text-zinc-600 italic px-2">No public web identity found for this number.</div>
        )}
      </SignalSection>
      ...
      <SignalSection title="Carrier Intelligence" icon={Database}>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
            ...
            <div className="text-zinc-200 font-bold text-sm">{caller.carrierData.operator}</div>
          </div>
          <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
            <div className="text-[10px] text-zinc-500 uppercase font-bold mb-1">State</div>
            <div className="text-zinc-200 font-bold text-sm">{caller.carrierData.state}</div>
          </div>
          <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
            <div className="text-[10px] text-zinc-500 uppercase font-bold mb-1">Type</div>
            <div className="text-zinc-200 font-bold text-sm uppercase">{caller.carrierData.numberType}</div>
          </div>
          <div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50">
