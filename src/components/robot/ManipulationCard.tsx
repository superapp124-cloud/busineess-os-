 ...
 * Dynamically synchronized with live MuJoCo physics and RobotCommandEngine.
 */
...
export const ManipulationCard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubTask = RobotCommandEngine.onTaskUpdate((t) => setActiveTask({ ...t }));
    return () => {
      unsubSim();
      unsubTask();
    };
  ...
  const graspForce = simState?.hand_contact_force_N ?? (activeTask.graspForceN ?? 0.0);
  const isContact = activeTask.contactConfirmed || graspForce > 0.5;
  const targetObject = activeTask.targetObject || (isContact ? 'water_bottle_01' : 'none');
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 flex flex-col gap-3 shadow-xl font-mono text-xs">
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-cyan-300 font-bold truncate">{targetObject}</span>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-slate-200 font-bold">{isContact ? '0.55 kg' : '0.00 kg'}</span>
        </div>
