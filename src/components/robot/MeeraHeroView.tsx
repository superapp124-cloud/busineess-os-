...
export const MeeraHeroView: React.FC<MeeraHeroViewProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubTask = RobotCommandEngine.onTaskUpdate((task) => {
      if (task.speechResponse) {
        setSpeechBubbleText(task.speechResponse);
      }
    });
    return () => {
      unsubVoice();
  ...
  const handleSpeakGreeting = useCallback(async () => {
    await meeraVoice.speak(speechBubbleText, 'hi-IN');
  }, [speechBubbleText]);

  // Actions connecting to RobotCommandEngine & MuJoCo
  const handleWave = async () => {
    setIsActionPending(true);
    try {
      await RobotCommandEngine.executeCommand('wave hello', 'hi-IN');
    } catch (e) {
  ...
  const handleHoldBottle = async () => {
    setIsActionPending(true);
    try {
      await RobotCommandEngine.executeCommand('kitchen se paani ki bottle lao', 'hi-IN');
    } catch (e) {
  ...
  const handleStand = async () => {
    setIsActionPending(true);
    try {
      await RobotCommandEngine.executeCommand('stand gracefully', 'hi-IN');
    } catch (e) {
  ...
  const handleWalk = async () => {
    setIsActionPending(true);
    try {
      await RobotCommandEngine.executeCommand('walk to kitchen', 'hi-IN');
    } catch (e) {
  ...
  const handlePush = async () => {
    setIsActionPending(true);
    try {
      await RobotCommandEngine.executeCommand('test push 450N', 'hi-IN');
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative w-full h-[400px] md:h-[440px] rounded-3xl overflow-hidden border border-slate-700/80 bg-slate-950 shadow-2xl flex flex-col justify-between p-5">
        ...
        <div className="relative z-10 max-w-[360px] bg-slate-900/90 backdrop-blur-md p-3.5 rounded-2xl border border-cyan-500/40 shadow-2xl flex flex-col gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        <button
          ...
          className="px-3 py-2.5 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-xl font-bold text-xs transition shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-50"
        >
          <span>👋</span>
        ...
        <button
          ...
          disabled={isActionPending}
          className="px-3 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl font-bold text-xs transition shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-50"
        >
          <span>🍶</span>
        ...
        <button
          onClick={handleStand}
          disabled={isActionPending}
          className="px-3 py-2.5 bg-gradient-to-r from-purple-700 to-indigo-700 hover:from-purple-600 hover:to-indigo-600 text-white rounded-xl font-bold text-xs transition shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-50"
        >
          <span>🧘</span>
        ...
        <button
          onClick={handleWalk}
          disabled={isActionPending}
          className="px-3 py-2.5 bg-gradient-to-r from-amber-700 to-yellow-700 hover:from-amber-600 hover:to-yellow-600 text-white rounded-xl font-bold text-xs transition shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-50"
        >
          <span>🚶</span>
        ...
        <button
          onClick={handlePush}
          disabled={isActionPending}
          className="px-3 py-2.5 bg-gradient-to-r from-rose-700 to-red-700 hover:from-rose-600 hover:to-red-600 text-white rounded-xl font-bold text-xs transition shadow-lg flex items-center justify-center gap-1.5 disabled:opacity-50 col-span-2 sm:col-span-1"
        >
          <span>⚡</span>
