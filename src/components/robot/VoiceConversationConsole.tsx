 ...
 * Fully wired to RobotCommandEngine for dynamic NLP intent recognition & robotics action dispatch.
 */
...
export const VoiceConversationConsole: React.FC<VoiceConversationConsoleProps> = ({ onSendCommand }) => {
  ...
  const getLangCode = () => {
    if (selectedLang.includes('English')) return 'en-IN';
    if (selectedLang.includes('Urdu')) return 'ur-PK';
    if (selectedLang.includes('Punjabi')) return 'pa-IN';
    if (selectedLang.includes('Bengali')) return 'bn-IN';
    if (selectedLang.includes('Tamil')) return 'ta-IN';
    if (selectedLang.includes('Telugu')) return 'te-IN';
    return 'hi-IN';
  };
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      // Execute command via RobotCommandEngine (Coordinates TTS, MuJoCo RPC, and Task steps)
      const activeTask = await RobotCommandEngine.executeCommand(text, getLangCode());

      const meeraMsg: Message = {
        id: String(Date.now() + 1),
        sender: 'meera',
        text: activeTask.speechResponse,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
