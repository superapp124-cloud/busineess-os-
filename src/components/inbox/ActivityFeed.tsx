      ...
      {Object.entries(grouped).map(([intentName, items]) => (
        <div key={intentName} className="space-y-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-indigo-400">Intent: {intentName}</span>
            <div className="h-px bg-white/10 flex-1" />
          </div>
                ...
                <div className={cn("rounded-2xl border p-4 backdrop-blur-xl transition-all duration-300",
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-white">{item.sender.name}</span>
                      <span className="text-xs text-slate-500">via {item.provider}</span>
                    </div>
                  ...
                  {item.aiSummary && (
                    <div className="flex gap-2 items-center text-xs font-medium text-indigo-300 mb-2">
                      <CheckCircle2 className="w-3 h-3" />
                      <span>{item.aiSummary}</span>
                    </div>
                  )}

                  <p className="text-slate-300 text-sm mb-4">{item.preview}</p>
