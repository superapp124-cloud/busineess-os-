  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center space-y-6 max-w-md w-full">
        <div>
          <h2 className="text-3xl font-bold text-white mb-2">{hubName} Hub</h2>
        ...
        <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 shadow-xl backdrop-blur-xl">
          <p className="text-sm font-semibold text-slate-300 uppercase tracking-widest mb-4">Connect an account</p>
          <div className="space-y-3">
            {providers.map(provider => (
              <Button 
                key={provider.id} 
                variant="outline" 
                className="w-full justify-start h-14 bg-white/5 border-white/10 hover:bg-white/10 text-white font-medium text-lg rounded-2xl"
                onClick={() => navigate('/desktop/connected-accounts')}
              >
                <div className={`w-8 h-8 rounded-full ${provider.color} flex items-center justify-center mr-3 text-sm font-bold shadow-inner`}>
