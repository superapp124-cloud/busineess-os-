    ...
    >
      <div className="p-4 border-b border-white/5 flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="text-white font-semibold text-lg">{insight.title}</div>
          <div className="text-white/60 text-sm mt-1">{insight.summary}</div>
          <div className="mt-2 text-xs bg-white/5 p-2 rounded-lg text-white/70 border border-white/5">
        ...
        {atsResult.missingSkills && atsResult.missingSkills.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-white/90">Missing Core Skills</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {atsResult.missingSkills.map((skill: string, i: number) => (
                <span key={i} className="px-2 py-1 rounded border border-red-500/20 bg-red-500/10 text-red-300 text-xs font-medium">
                  {skill}
                </span>
              ))}
            </div>
        ...
        {atsResult.strengths && atsResult.strengths.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-white/90">Key Strengths</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {atsResult.strengths.map((skill: string, i: number) => (
                <span key={i} className="px-2 py-1 rounded border border-emerald-500/20 bg-emerald-500/10 text-emerald-300 text-xs font-medium">
                  {skill}
                </span>
              ))}
            </div>
        ...
        {atsResult.interviewQuestions && atsResult.interviewQuestions.length > 0 && (
          <div className="pt-2 border-t border-white/5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-white/90">Recommended Interview Questions</span>
            </div>
            <ul className="space-y-2">
              {atsResult.interviewQuestions.map((q: string, i: number) => (
                <li key={i} className="flex gap-2 text-sm text-white/70 bg-white/5 p-2 rounded-lg border border-white/5">
