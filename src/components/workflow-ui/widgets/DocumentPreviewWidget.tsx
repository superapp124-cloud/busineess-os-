      ...
      {documents.map((doc: any, i: number) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-white truncate">{doc.metadata?.fileName || 'Document'}</div>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-xs text-white/50 uppercase">{doc.classification?.primary || 'Unknown'}</span>
              <span className="text-white/20 text-xs">•</span>
              <span className="text-xs text-white/50">{doc.pages} page(s)</span>
            </div>
          </div>
        </motion.div>
      ))}
