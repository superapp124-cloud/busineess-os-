  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 border-b border-white/5 pb-3 relative z-10">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 border border-emerald-500/20 flex items-center justify-center text-2xl shadow-inner">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-white/90 font-medium leading-tight mt-0.5">{outcome.title}</p>
        </div>
      </div>
      ...
      {outcome.status === 'suggested' && (
        ...
        >
          <button 
            ...
            className="flex-1 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-900/20 disabled:opacity-50 disabled:shadow-none"
          >
            {isExecuting ? 'Confirming...' : 'Confirm'} <Check className="w-3.5 h-3.5" />
          </button>
          <button 
            onClick={handleCancel}
            className="px-4 py-2.5 bg-zinc-800/50 hover:bg-zinc-700/50 text-white/70 hover:text-white text-xs font-semibold rounded-xl transition-all border border-white/5"
          >
      ...
      {(outcome.status === 'extracting' || outcome.status === 'executing' || outcome.status === 'waiting' || outcome.status === 'observed') && (
        <motion.div 
          ...
          className="py-4 px-3 bg-zinc-800/30 rounded-xl flex items-center justify-center text-white/70 text-xs font-medium border border-white/5 relative z-10"
        >
      ...
      {outcome.status === 'searching' && (
        ...
        >
          ...
          <p className="text-xs text-white/60 font-medium tracking-wide">Searching providers...</p>
        </motion.div>
      )}
      ...
      {outcome.status === 'completed' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-2.5 px-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold rounded-xl flex items-center justify-center relative z-10">
          <Check className="w-3.5 h-3.5 mr-2" /> Completed
        </motion.div>
      )}
      
      {outcome.status === 'canceled' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-2.5 px-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold rounded-xl flex items-center justify-center relative z-10">
