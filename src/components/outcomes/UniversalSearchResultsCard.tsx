...
export const UniversalSearchResultsCard: React.FC<UniversalSearchResultsCardProps> = ({ commitment, onSelect }) => {
  ...
  if (results.length === 0) {
    return <div className="text-white/50 text-xs p-3">No results found from providers.</div>;
  }
                ...
                <div className="flex justify-between items-start">
                  {config.columns.map(col => (
                    <div key={col.key} className="flex flex-col">
                      ...
                      <span className={`text-sm font-bold ${col.type === 'currency' ? 'text-emerald-400' : 'text-white/90'}`}>
                        {result[col.key]}
                      </span>
                    </div>
                  ))}
