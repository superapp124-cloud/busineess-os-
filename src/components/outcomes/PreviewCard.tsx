  ...
  return (
    <div className="bg-zinc-800/40 rounded-lg mt-2 p-3 border border-zinc-700/50">
      <div className="flex items-center gap-2 mb-3">
        {preview.icon && <span className="text-xl">{preview.icon}</span>}
        <h4 className="text-sm font-semibold text-white/90">{preview.title}</h4>
      </div>
      ...
      <div className="flex gap-2 mt-2">
        <button 
          ...
          className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
        >
        ...
        <button 
          onClick={onCancel}
          disabled={isExecuting}
          className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-white/70 text-xs font-semibold rounded-lg transition-colors"
        >
          <X className="w-3 h-3" />
        </button>
      </div>
