...
export const MissingFieldsCard: React.FC<MissingFieldsCardProps> = ({ missingFields, onSelect }) => {
  ...
  return (
    <div className="bg-zinc-800/50 p-3 rounded-lg mt-2 border border-zinc-700/50">
      <p className="text-sm text-white/90 font-medium mb-3">{field.label}</p>
        ...
        <div className="flex flex-wrap gap-2">
          {field.options.map(option => (
            <button
              ...
              className="px-3 py-1.5 bg-zinc-700 hover:bg-zinc-600 text-white text-xs font-medium rounded-full transition-colors border border-zinc-600"
            >
              {option}
            </button>
          ))}
      ...
      {field.type === 'text' && (
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-zinc-900 border border-zinc-700 text-white text-sm rounded-lg px-3 py-1.5 outline-none focus:border-emerald-500"
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
