    ...
    <div className="fixed inset-0 z-[240] flex flex-col bg-black">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">{getTimeAgo(currentStory.created_at)}</p>
            </div>
          </div>
      ...
      <div className="relative flex-1">
        ...
        {(currentStory.media_type === 'text' || currentStory.media_url?.startsWith('data:image/gif')) ? (
          templateId === 'brand-card' ? (
            <div className="flex h-full flex-col items-center justify-center p-8 text-center relative border-[12px] border-amber-500/20" style={{ background: 'linear-gradient(135deg, #1e1b4b 0%, #311042 50%, #030008 100%)' }}>
              <div className="absolute top-28 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold uppercase tracking-[0.2em] px-4 py-1.5 rounded-full shadow-lg">
                💼 BRAND PRESENTATION CARD
              </div>
              <p className="max-w-lg text-3xl font-bold leading-relaxed text-amber-50 drop-shadow-md">
                {captionText}
              </p>
            </div>
            ...
            <div className="flex h-full flex-col items-center justify-center p-8 text-center relative border-[12px] border-rose-500/20" style={{ background: 'linear-gradient(135deg, #be185d 0%, #6b072e 50%, #24000c 100%)' }}>
              <div className="absolute top-28 bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-bold uppercase tracking-[0.2em] px-4 py-1.5 rounded-full shadow-lg">
                🔥 TODAY'S EXCLUSIVE PROMO
              </div>
              <p className="max-w-lg text-3xl font-black leading-relaxed text-rose-50 drop-shadow-md">
                {captionText}
              </p>
            </div>
            ...
            <div className="flex h-full flex-col items-center justify-center p-8 text-center relative border-[12px] border-violet-500/20" style={{ background: 'linear-gradient(135deg, #4c1d95 0%, #1e1b4b 50%, #090514 100%)' }}>
              <div className="absolute top-28 bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs font-bold uppercase tracking-[0.2em] px-4 py-1.5 rounded-full shadow-lg">
                ⭐️⭐️⭐️⭐️⭐️ 5-STAR TESTIMONIAL
              </div>
              <span className="text-[5rem] text-violet-400/20 absolute font-serif left-8 top-44">“</span>
              <p className="max-w-lg text-2xl font-medium italic leading-relaxed text-violet-100 relative z-10 px-4 drop-shadow-md">
                {captionText}
              </p>
            ...
            <div className="flex h-full flex-col items-center justify-center p-8 text-center relative border-[12px] border-emerald-500/20" style={{ background: 'linear-gradient(135deg, #065f46 0%, #064e3b 50%, #022c22 100%)' }}>
              <div className="absolute top-28 bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-bold uppercase tracking-[0.2em] px-4 py-1.5 rounded-full shadow-lg">
                📅 NOW BOOKING APPOINTMENTS
              </div>
              <p className="max-w-lg text-3xl font-extrabold leading-relaxed text-emerald-50 drop-shadow-md">
                {captionText}
              </p>
            </div>
          ) : (
            <div className="flex h-full items-center justify-center bg-[linear-gradient(160deg,#2b1f53_0%,#5b2ad1_44%,#1a0e39_100%)] p-8">
              <p className="max-w-lg text-center text-3xl font-medium leading-relaxed text-white">
                {captionText}
              </p>
            </div>
          )
        ...
        {captionText && currentStory.media_type !== 'text' && !currentStory.media_url?.startsWith('data:image/gif') && (
          ...
          >
            <p className="text-center text-lg text-white drop-shadow-lg">{captionText}</p>
          </div>
        )}
