              ...
              <ScrollArea className="flex-1">
                <div className="px-4 py-3 space-y-3">
                  {chatMessages.length === 0 && (
                    <p className="text-center text-xs text-white/30 py-4">No messages yet. Say hello! 👋</p>
                  )}
                  {chatMessages.map((msg) => (
                    <div key={msg.id} className={cn('flex items-start gap-2', msg.self && 'flex-row-reverse')}>
                      ...
                      <div className={cn('max-w-[70%]', msg.self && 'items-end flex flex-col')}>
                        ...
                        <div className={cn('px-3 py-2 rounded-2xl text-xs', msg.self ? 'bg-purple-600/80 text-white' : 'bg-white/[0.06] text-white/80')}>
                          {msg.text}
                        </div>
          ...
          {activeTab === 1 && (
            <ScrollArea className="flex-1">
              <div className="px-4 py-3 space-y-2">
                ...
                {logEvents.length === 0 && (
                  <p className="text-xs text-white/30 text-center py-4">No activity recorded yet.</p>
                )}
                {logEvents.map((event, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className={cn('text-xs', LOG_COLORS[event.type])}>{event.message}</span>
                  </div>
                ))}
              </div>
            </ScrollArea>
            ...
            <ScrollArea className="flex-1">
              <div className="px-4 py-3 space-y-2">
                ...
                {sharedFiles.map((file) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center text-lg shrink-0 bg-purple-500/10 border border-purple-500/20">
                      📎
                    </div>
              ...
              <div className="px-4 py-3">
                ...
                {waitingRoom.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-xs text-white/30">No one is waiting</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {waitingRoom.map((person) => (
                      <div key={person.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.03] border border-amber-500/20">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0" style={{ background: person.avatarColor }}>
                          {person.name[0]}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-medium text-white/90 truncate">{person.name}</div>
