  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-900/50">
      <div className="px-4 py-3 border-b border-white/[0.05] flex items-center justify-between">
        ...
        {isHost && !showCreate && (
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-1 px-2 py-1 bg-purple-600 hover:bg-purple-500 rounded text-xs font-bold text-white transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {showCreate && isHost && (
          <form onSubmit={handleCreatePoll} className="bg-white/[0.03] border border-purple-500/30 rounded-xl p-4 space-y-3">
            <h4 className="text-xs font-bold text-white/70">Create New Poll</h4>
            <input 
            ...
            {options.length < 5 && (
              <button type="button" onClick={() => setOptions([...options, ''])} className="text-xs text-purple-400 hover:text-purple-300 font-medium">
                + Add Option
              </button>
            )}
            <div className="flex gap-2 pt-2">
              <button type="submit" className="flex-1 py-1.5 bg-purple-600 hover:bg-purple-500 rounded-lg text-xs font-bold text-white transition-colors">Publish Poll</button>
              <button type="button" onClick={() => setShowCreate(false)} className="px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-xs font-medium text-white transition-colors">Cancel</button>
            </div>
          </form>
        )}
        ...
        {polls.length === 0 && !showCreate && (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {isHost && <p className="text-xs text-white/30 mt-1">Create one to engage your audience.</p>}
          </div>
        )}
        ...
        {polls.map(poll => {
          ...
          return (
            <div key={poll.id} className="bg-white/[0.03] border border-white/[0.05] rounded-xl p-4 relative group">
              ...
              <h4 className="text-sm font-semibold text-white/90 mb-4 pr-6">{poll.question}</h4>
              <div className="space-y-2">
                    ...
                    >
                      ...
                      <div className="relative px-3 py-2 flex items-center justify-between z-10">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-white/80">{opt}</span>
                        </div>
                        {totalVotes > 0 && <span className="text-xs text-white/50">{percent}%</span>}
                      </div>
                    </button>
                  );
                })}
