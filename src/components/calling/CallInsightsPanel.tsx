        ...
        <div className="px-6 pb-8 space-y-5 max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-muted/50">
            <div className="flex-1">
              <p className="font-semibold text-sm">{callerInfo?.name ?? 'Looking up...'}</p>
              <p className="text-xs text-muted-foreground">{phoneNumber}</p>
            </div>
            {callerInfo && (
              <span className={`text-xs font-semibold px-2 py-1 rounded-full border capitalize ${riskColor[callerInfo.riskLevel]}`}>
            ...
            <div className="rounded-2xl bg-primary/5 border border-primary/20 p-4">
              ...
              {suggestedAction && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-green-700">{suggestedAction}</p>
                </div>
              )}
            </div>
          ) : null}
          ...
          <div>
            ...
            <textarea
              className="w-full rounded-xl border bg-background p-3 text-sm placeholder:text-muted-foreground focus:ring-2 focus:ring-primary outline-none resize-none"
              rows={3}
          ...
          {/* Save */}
          <Button className="w-full rounded-xl h-12 text-sm font-semibold" onClick={handleSave} disabled={saving}>
