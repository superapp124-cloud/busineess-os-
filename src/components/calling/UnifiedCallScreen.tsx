      ...
      <AnimatePresence>
        {lossBridgeMsg && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-20 inset-x-4 mx-auto max-w-sm z-50 flex items-center gap-2.5 px-4 py-3 rounded-2xl bg-amber-500/90 text-white font-semibold text-sm shadow-xl backdrop-blur-xl border border-amber-400/40"
          >
      ...
      <AnimatePresence>
        {showKeypad && (
          ...
          >
            <div className="mb-4 rounded-2xl bg-white/6 border border-white/8 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 min-h-[28px] text-xl font-medium tracking-[0.24em] text-white font-mono break-all">
                    {dtmfInput || '—'}
                  </p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-xs mx-auto">
              {keypadDigits.map(d => (
                <button
                  ...
                  className="w-16 h-16 rounded-full bg-white/10 border border-white/10 text-white text-2xl font-semibold active:bg-white/20 active:scale-95 transition-all mx-auto flex items-center justify-center"
                >
                  {d}
                </button>
              ))}
            ...
            <button
              ...
              className="w-full mt-4 rounded-full bg-white/8 py-3 text-sm font-medium text-emerald-300 active:scale-[0.99] transition-transform"
            >
              Hide Keypad
            </button>
          </motion.div>
        ...
        {isTranslating && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50">
            <div className={`px-4 py-2 rounded-full text-white text-sm font-medium shadow-lg backdrop-blur-sm flex items-center gap-2 ${aiError ? 'bg-red-500/90' : !isConnected || !isReady ? 'bg-amber-500/90 animate-pulse' : 'bg-purple-600/90'}`}>
              {aiError ? (
                <>
