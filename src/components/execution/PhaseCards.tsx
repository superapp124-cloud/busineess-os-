      ...
      <div className="w-full p-6 rounded-2xl border border-violet-500/30 bg-violet-500/5 animate-in fade-in duration-300">
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">{formatPrice(amount, currency)} via {selectedMethod.toUpperCase()}</p>
          </div>
      ...
      <div className="w-full p-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 animate-in fade-in zoom-in-95 duration-400">
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">{formatPrice(amount, currency)} paid</p>
            {txnId && <p className="text-xs text-muted-foreground/70 mt-1 font-mono">Txn: {txnId}</p>}
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full p-6 rounded-2xl border border-red-500/30 bg-red-500/5 animate-in fade-in duration-300">
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Please try again or use a different method</p>
          </div>
          <button onClick={() => onPay(selectedMethod)} className="bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 px-4 py-2 rounded-xl text-sm font-medium transition-colors">
            Try Again
          </button>
        </div>
      </div>
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="p-5 rounded-2xl border border-border/50 bg-card/60 mb-3">
        <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-1">Total Amount</p>
        <div className="flex items-baseline gap-2">
          <span className="text-3xl font-bold text-foreground">{formatPrice(amount, currency)}</span>
      ...
      <div className="flex gap-2">
        <button
          onClick={onCancel}
          className="flex-shrink-0 px-4 py-2.5 rounded-xl border border-border/60 text-muted-foreground text-sm hover:bg-muted/40 transition-colors"
        >
          Cancel
        </button>
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white py-2.5 rounded-xl text-sm font-semibold transition-all shadow-lg hover:shadow-violet-500/20 active:scale-[0.98]"
        >
...
export function SuccessCard({ bookingId, pnr, title, subtitle, details = [], onAddCalendar, onSetReminder, onShare }: SuccessCardProps) {
  return (
    <div className="w-full animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="p-6 rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 to-violet-500/5 mb-3">
        ...
        {pnr && (
          <div className="bg-card/60 rounded-xl border border-border/40 p-3 mb-3">
            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">PNR Number</p>
            <p className="text-2xl font-mono font-bold text-foreground tracking-wider">{pnr}</p>
          </div>
        )}
      ...
      <div className="flex gap-2">
        {onAddCalendar && (
          <button onClick={onAddCalendar} className="flex-1 text-xs py-2.5 rounded-xl border border-border/50 bg-card/40 hover:bg-card/70 text-muted-foreground hover:text-foreground transition-colors">
            📅 Calendar
          </button>
        )}
        {onSetReminder && (
          <button onClick={onSetReminder} className="flex-1 text-xs py-2.5 rounded-xl border border-border/50 bg-card/40 hover:bg-card/70 text-muted-foreground hover:text-foreground transition-colors">
            🔔 Remind me
          </button>
        )}
        {onShare && (
          <button onClick={onShare} className="flex-1 text-xs py-2.5 rounded-xl border border-border/50 bg-card/40 hover:bg-card/70 text-muted-foreground hover:text-foreground transition-colors">
            📤 Share
          </button>
        )}
      </div>
