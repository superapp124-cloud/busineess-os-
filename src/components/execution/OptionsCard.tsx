  ...
  return (
    ...
    `}>
      ...
      <div className="flex items-center gap-4 p-4">
        {/* Provider icon */}
        <div className="w-10 h-10 rounded-xl bg-muted/60 border border-border/40 flex items-center justify-center text-xl shrink-0">
          {emoji}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-muted-foreground mt-0.5 truncate">{option.subtitle || option.providerName}</div>

          {/* Train: departure → arrival */}
          {train && option.departureTime && (
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm font-semibold text-foreground">{formatTime(option.departureTime)}</span>
              ...
              <span className="text-sm font-semibold text-foreground">{formatTime(option.arrivalTime)}</span>
            </div>
          )}
  ...
  return (
    <div className="w-full p-5 rounded-2xl border border-border/50 bg-card/60 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap gap-2">
        {providers.map((p, i) => (
          <span key={p} className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border ${i === 0 ? 'bg-violet-500/10 border-violet-500/30 text-violet-400' : 'bg-muted/40 border-border/40 text-muted-foreground'}`}>
