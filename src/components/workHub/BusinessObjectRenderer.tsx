  ...
  return (
    <div className="bg-black/40 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">{definition.name}</h2>
          <p className="text-sm text-slate-400 mt-1">Complete the required fields below.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {Object.entries(definition.fields.properties).map(([fieldName, fieldSchema], idx) => {
          ...
          return (
            ...
            >
              <label className="text-sm font-semibold text-slate-300 mb-2 capitalize tracking-wide flex items-center justify-between">
                <span>
        ...
        {!readOnly && (
          ...
          >
            <button
              type="submit"
              className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:shadow-[0_0_30px_rgba(124,58,237,0.4)] hover:scale-[1.02] transition-all w-full flex items-center justify-center gap-2"
            >
