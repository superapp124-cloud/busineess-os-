...
export const EnterpriseCanvas: React.FC<Props> = ({ missionContext, mode, isProcessing }) => {
  ...
  if (!missionContext) {
    return (
      <div className="flex-1 flex flex-col bg-slate-50 relative overflow-hidden">
        <ContextBar breadcrumbs={['Enterprise', 'Idle']} />
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 animate-in fade-in duration-500">
          ...
          <h2 className="text-lg font-bold text-slate-800 mb-2 tracking-tight">Enterprise Canvas</h2>
          <p className="text-slate-500 text-sm max-w-sm mx-auto leading-relaxed">
            Select an enterprise object from the navigator or hit{' '}
            <kbd className="font-mono bg-slate-200 px-1 py-0.5 rounded text-slate-600 text-[10px]">⌘K</kbd> to search.
          </p>
        </div>
      </div>
    );
  ...
  const breadcrumbs = [
    ...
    missionContext.lifecycleState,
    `${missionContext.recommendations?.length || 0} Recommended Actions`,
  ];
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 relative overflow-hidden h-full">
      <ContextBar breadcrumbs={breadcrumbs} />
      ...
      <div className="flex-1 overflow-y-auto flex flex-col relative">
        ...
        {/* ─── DECISION MODE ───────────────────────────────────────────────── */}
        {mode === 'Decision' && (
          <div className="max-w-3xl mx-auto w-full p-6 flex flex-col gap-5 animate-in slide-in-from-bottom-4 duration-500">

            {/* 1. MISSION SUMMARY CARD — First thing users see */}
            <MissionSummaryCard missionContext={missionContext} />

            {/* 2. PIPELINE — Simplified by default, expandable to technical view */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-slate-700">Processing Pipeline</span>
                  <div className="flex items-center gap-1">
                    {['Analyzing', 'Review Needed', 'Waiting Approval', 'Processing', 'Complete'].slice(0, 3).map((stage, idx) => (
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-400 font-medium">
                    {showTechnicalPipeline ? 'Hide technical view' : 'Show technical view'}
                  </span>
                  {showTechnicalPipeline ? <ChevronUp className="w-3 h-3 text-slate-400" /> : <ChevronDown className="w-3 h-3 text-slate-400" />}
                </div>
              </button>

              {showTechnicalPipeline && (
                <div className="border-t border-slate-100 px-5 py-4 animate-in slide-in-from-top-2 duration-200">
                  <div className="flex items-center min-w-max overflow-x-auto pb-2 gap-1">
                    {pipelineSteps.map((step, idx) => (
                      <React.Fragment key={step.layer}>
                        {/* Clickable Step */}
                        <button
                          onClick={() => setExpandedStep(expandedStep === idx ? null : idx)}
                          className={`flex flex-col items-center gap-1.5 group min-w-[60px] ${expandedStep === idx ? 'opacity-100' : 'opacity-80 hover:opacity-100'}`}
                        >
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center border text-xs font-bold transition-all ${
                            step.status === 'Completed' ? 'bg-emerald-50 border-emerald-200 text-emerald-600 group-hover:border-emerald-400' :
                            step.status === 'Waiting'   ? 'bg-amber-50 border-amber-200 text-amber-600 group-hover:border-amber-400' :
                            step.status === 'Running'   ? 'bg-indigo-50 border-indigo-200 text-indigo-600' :
                            'bg-slate-50 border-slate-200 text-slate-400'
                          } ${expandedStep === idx ? 'ring-2 ring-indigo-400 ring-offset-1' : ''}`}>
                            {step.status === 'Completed' ? <CheckCircle className="w-4 h-4" /> :
                             step.status === 'Running'   ? <Loader2 className="w-4 h-4 animate-spin" /> :
                             idx + 1}
                          </div>
                          <div className="text-center">
                            <div className="text-[9px] font-bold text-slate-700">{step.layer}</div>
                        ...
                        {idx < pipelineSteps.length - 1 && (
                          <div className={`w-4 h-px shrink-0 ${
                            step.status === 'Completed' && pipelineSteps[idx + 1].status !== 'Pending' ? 'bg-emerald-200' : 'bg-slate-200'
                          }`} />
                        )}
                      </React.Fragment>
                    ))}
                  ...
                  {expandedStep !== null && (
                    <div className="mt-3 bg-slate-50 border border-slate-200 rounded-xl p-3 animate-in slide-in-from-top-1 duration-150">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold text-slate-800">{pipelineSteps[expandedStep].layer}</span>
                        {pipelineSteps[expandedStep].duration !== '—' && (
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2 text-[10px]">
                          ...
                          <span className="font-mono font-semibold text-slate-700">{pipelineSteps[expandedStep].plugin}</span>
                        </div>
                        <div className="flex items-start gap-2 text-[10px]">
                          <GitBranch className="w-3 h-3 text-slate-400 mt-0.5" />
                          <span className="text-slate-500">Output:</span>
                          <span className="text-slate-700">{pipelineSteps[expandedStep].output}</span>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {/* 3. DECISION CARDS — Rich with Why/Impact/Evidence */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h1 className="text-base font-bold text-slate-900 tracking-tight">{missionContext.mission}</h1>
                  <p className="text-xs text-slate-500 mt-0.5">{missionContext.actionRequired}</p>
                </div>
              </div>

              {missionContext.recommendations && missionContext.recommendations.length > 0 ? (
                <div className="space-y-3 mt-4">
                  {missionContext.recommendations.map((rec: any, idx: number) => (
                    <RichDecisionCard key={idx} rec={rec} />
                  ))}
                </div>
              ) : (
                <div className="mt-4 p-6 border border-dashed border-slate-200 rounded-xl flex items-center justify-center text-sm text-slate-500">
                  No automated decisions reached. Manual review required.
                </div>
              )}
            </div>

            {/* 4. EVIDENCE — Larger, more prominent */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
                <FileText className="w-3.5 h-3.5" /> Evidence Source
              </h3>
              <div className="rounded-xl overflow-hidden border border-slate-200 relative" style={{ height: '220px' }}>
                <div className="absolute inset-0 bg-gradient-to-b from-transparent to-white/90 z-10 pointer-events-none flex items-end justify-center pb-4">
                  <button className="px-4 py-2 bg-white border border-slate-200 rounded-full shadow-sm text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                    Expand Evidence ⌘E
                  </button>
                </div>
                <WorkspaceViewport item={mockItem} />
