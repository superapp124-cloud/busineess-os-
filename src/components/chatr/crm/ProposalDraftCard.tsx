  ...
  return (
    <div className="w-full max-w-md bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      ...
      <div className="p-4 border-b border-slate-700/50 bg-violet-950/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">Valid until {data.validUntil}</p>
          </div>
        </div>
      ...
      <div className="p-4 border-b border-slate-700/50">
        <p className="text-xs text-slate-400 mb-2 font-medium">Executive Summary</p>
        <p className="text-sm text-slate-300 leading-relaxed line-clamp-3">{data.executiveSummary}</p>
      </div>
      ...
      <div className="border-b border-slate-700/50">
        <button onClick={() => setExpanded(v => !v)}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-slate-200">Pricing Breakdown</span>
          </div>
        ...
        {expanded && (
          <div className="px-4 pb-4 space-y-2">
            ...
            {(data.discountPercentage || 0) > 0 && (
              <div className="flex justify-between items-center text-sm text-rose-400 pt-1 border-t border-slate-700/50">
      ...
      <div className="p-4 bg-slate-950/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xl font-bold text-violet-400">₹{discounted.toLocaleString()}</p>
        </div>
        {data.status === 'DRAFT' && (
