    ...
    <div className="w-full max-w-md bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      ...
      <div className="p-4 border-b border-slate-700/50 bg-sky-950/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">{itinerary.travelerName} · {flight?.departureDate}</p>
          </div>
        </div>
      ...
      {itinerary.summary && (
        <div className="px-4 py-3 bg-slate-800/20 border-b border-slate-700/50">
          ...
          <p className="text-sm text-slate-300 leading-relaxed">{itinerary.summary}</p>
        </div>
      )}
      ...
      <div className="p-4 space-y-3 border-b border-slate-700/50">
        ...
        {flight && (
          <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3 border border-slate-700/40">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-200 font-medium">{flight.airline} {flight.flightNumber}</p>
              <p className="text-xs text-slate-400">{flight.seatClass} · {flight.departureDate}</p>
            </div>
        ...
        {hotel && (
          <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3 border border-slate-700/40">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-200 font-medium">{hotel.hotelName}</p>
              <p className="text-xs text-slate-400">{hotel.checkIn} → {hotel.checkOut}</p>
            </div>
            <div className="flex flex-col items-end">
        ...
        {taxi && (
          <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl p-3 border border-slate-700/40">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-200 font-medium">{taxi.provider}</p>
              <p className="text-xs text-slate-400">{taxi.pickupLocation}</p>
            </div>
            <div className="flex flex-col items-end">
