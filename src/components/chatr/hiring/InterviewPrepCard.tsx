      ...
      <div className="p-4 border-b border-slate-700/50 bg-slate-800/50">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">AI Generated based on Skill Gaps</p>
          </div>
        </div>
          ...
          <div key={idx} className="border-b border-slate-700/50 last:border-0">
            <button onClick={() => toggle(idx)} className="w-full p-4 flex items-center justify-between hover:bg-slate-800/30 transition-colors">
              <div className="flex flex-col items-start">
                <span className="text-sm font-medium text-slate-200">{section.type} Interview</span>
                <span className="text-xs text-slate-400 mt-0.5">Focus: {section.focus}</span>
              </div>
            ...
            {expandedSection === idx && (
              <div className="px-4 pb-4 space-y-2">
                {section.questions.map((q, qIdx) => (
                  <div key={qIdx} className="bg-slate-950/50 rounded-lg p-3 text-sm text-slate-300 flex gap-3">
