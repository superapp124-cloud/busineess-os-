    ...
    <div className="w-full max-w-2xl bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      ...
      <div className="p-4 border-b border-slate-700/50 bg-slate-800/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">{data.contact} • {data.experienceYears}y exp</p>
          </div>
        </div>
        <div className="text-right">
          <div className="flex items-center gap-1.5 justify-end">
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
            <span className="text-emerald-400 font-medium text-sm">{Math.round(data.confidence * 100)}% Confidence</span>
          </div>
          <p className="text-slate-500 text-xs mt-1">Enterprise Policy Checked</p>
        </div>
      </div>
      ...
      <div className="border-b border-slate-700/50">
        <button onClick={() => toggle('overview')} className="w-full p-4 flex items-center justify-between hover:bg-slate-800/30 transition-colors">
          <span className="text-sm font-medium text-slate-200">Candidate Overview</span>
          ...
          <div className="px-4 pb-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-800/50 flex items-center justify-between">
                ...
                <span className={cn("text-lg font-bold", data.matchPercentage > 80 ? "text-emerald-400" : "text-amber-400")}>
                  {data.matchPercentage}%
                </span>
              </div>
              <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-800/50 flex items-center justify-between">
                <span className="text-xs text-slate-400">Recommendation</span>
                <span className="text-xs font-bold text-white bg-indigo-500 px-2 py-1 rounded-md">{data.recommendation}</span>
              </div>
            </div>
            <div className="mt-4 bg-indigo-900/10 border border-indigo-500/20 rounded-xl p-3">
              <p className="text-xs text-indigo-200/70 font-medium mb-1">Decision Reasoning</p>
              <p className="text-sm text-indigo-100">{data.reasoning}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="border-b border-slate-700/50">
        <button onClick={() => toggle('skills')} className="w-full p-4 flex items-center justify-between hover:bg-slate-800/30 transition-colors">
          <span className="text-sm font-medium text-slate-200">Skills & Gaps Analysis</span>
        ...
        {expandedSection === 'skills' && (
          <div className="px-4 pb-4 space-y-4">
            <div>
              <p className="text-xs text-slate-400 mb-2 flex items-center gap-1.5"><CheckCircle className="w-3 h-3 text-emerald-400" /> Verified Skills</p>
            ...
            <div>
              <p className="text-xs text-slate-400 mb-2 flex items-center gap-1.5"><AlertTriangle className="w-3 h-3 text-amber-400" /> Missing / Unverified Skills</p>
              <div className="flex flex-wrap gap-1.5">
