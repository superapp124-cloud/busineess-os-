      ...
      <div className="p-4 border-b border-slate-700/50 bg-indigo-950/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">Leave Request: {data.leaveType}</p>
          </div>
        </div>
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-200">{data.startDate}</p>
          </div>
          <div>
            <p className="text-[11px] text-slate-500 uppercase font-bold tracking-wider mb-1">End Date</p>
            <p className="text-sm text-slate-200">{data.endDate}</p>
          </div>
        </div>
        <div>
          <p className="text-[11px] text-slate-500 uppercase font-bold tracking-wider mb-1">Reason</p>
          <p className="text-sm text-slate-300 bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">{data.reason}</p>
        </div>
      </div>
      {data.status === 'PENDING' && (
        <div className="p-3 bg-slate-950/50 border-t border-slate-800 flex gap-2">
          <button onClick={onApprove} className="flex-1 py-2.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors">
            <CheckCircle className="w-4 h-4" /> Approve
          </button>
          <button onClick={onReject} className="flex-1 py-2.5 bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-500/30 text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors">
