  ...
  return (
    <div className="w-full max-w-sm bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      <div className="p-4 border-b border-slate-700/50 bg-emerald-950/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">{data.month} {data.year}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-950/50 flex justify-between items-center">
        <span className="text-sm font-bold text-slate-300 uppercase tracking-wider">Net Pay</span>
        <span className="text-xl font-bold text-emerald-400">₹{data.netPay.toLocaleString()}</span>
      </div>
      <div className="p-3 bg-slate-950/50 border-t border-slate-800">
        <button className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors border border-slate-700">
