    ...
    <div className="w-full max-w-sm bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      ...
      <div className="p-4 border-b border-slate-700/50 bg-slate-800/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-sm">Expense Report</h3>
            <p className="text-slate-400 text-xs">{data.employeeName} · {data.category}</p>
          </div>
        </div>
      ...
      <div className="p-4 border-b border-slate-700/50">
        <div className="flex items-end justify-between mb-3">
          <div>
            ...
            <p className="text-2xl font-bold text-white">{data.amount ? formatMoney(data.amount) : '—'}</p>
          </div>
          <div className={cn('flex items-center gap-1.5 text-xs font-medium', policyColor)}>
      ...
      {data.status === 'SUBMITTED' && onApprove && (
        <div className="p-3 bg-slate-950/50 border-t border-slate-800">
          <button onClick={onApprove}
            className="w-full py-2.5 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors">
