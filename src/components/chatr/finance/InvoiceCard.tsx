    ...
    <div className="w-full max-w-sm bg-slate-900 border border-slate-700/50 rounded-2xl overflow-hidden shadow-xl mb-4 font-sans">
      ...
      <div className="p-4 border-b border-slate-700/50 bg-blue-950/20 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-sm">{data.invoiceNumber}</h3>
            <p className="text-slate-400 text-xs">{data.clientName} · Due {data.dueDate}</p>
          </div>
        </div>
      ...
      <div className="p-4 border-b border-slate-700/50 space-y-2">
        ...
        <div className="flex justify-between text-sm pt-2 border-t border-slate-700/50 font-bold">
          <span className="text-slate-200">Total</span>
          <span className="text-blue-400 text-base">{data.totalAmount ? formatMoney(data.totalAmount) : '—'}</span>
        </div>
      </div>
      ...
      {data.status === 'SENT' && onMarkPaid && (
        <div className="p-3 bg-slate-950/50">
          <button onClick={onMarkPaid}
            className="w-full py-2.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 border border-blue-500/30 text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors">
