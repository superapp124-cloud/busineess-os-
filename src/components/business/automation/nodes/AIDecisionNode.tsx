    ...
    <div className={`px-4 py-3 shadow-md rounded-xl bg-white dark:bg-[#1A1F2E] border-2 transition-all ${selected ? 'border-primary shadow-primary/20' : 'border-gray-200 dark:border-white/10'}`}>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-semibold uppercase tracking-wider text-purple-500">AI Decision</div>
            <div className="text-sm font-medium text-gray-900 dark:text-white mt-0.5">{data.label as string}</div>
          </div>
        </div>
        
        {data.prompt && (
          <div className="mt-2 text-xs text-gray-500 dark:text-white/60 bg-gray-50 dark:bg-black/20 p-2 rounded-md border border-gray-100 dark:border-white/5 line-clamp-2">
