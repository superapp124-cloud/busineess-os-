      ...
      <DialogContent className="sm:max-w-3xl p-0 gap-0 overflow-hidden bg-[#FAFAFA] border-none shadow-2xl rounded-2xl">
        <form onSubmit={handleSearch} className="flex items-center p-4 border-b bg-white">
          ...
          <Input
            ...
            className="flex-1 border-none bg-transparent text-lg shadow-none focus-visible:ring-0 placeholder:text-gray-400 p-0 h-10"
          />
          {query && (
            <Button 
              type="button" 
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-gray-50/80 border-b border-gray-100 text-xs overflow-x-auto">
          ...
          <Button variant="ghost" size="sm" onClick={handleBackfill} className="text-xs h-7 text-muted-foreground hover:text-foreground">
            Index History
          </Button>
        </div>
        ...
        <ScrollArea className="max-h-[60vh] min-h-[300px]">
          {isSearching ? (
            <div className="flex flex-col items-center justify-center h-48 text-gray-400 gap-3">
              ...
              <p className="text-sm font-medium">Scanning your knowledge graph...</p>
            </div>
          ) : answer ? (
            <div className="p-6">
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            <div className="mt-0.5 shrink-0 bg-violet-100 w-6 h-6 rounded-md flex items-center justify-center text-violet-600 font-mono text-xs font-bold">
                              {idx + 1}
                            </div>
            ...
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400 p-8 text-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm">Search across messages, images, documents, and voice notes instantly using semantic AI.</p>
              </div>
            </div>
          )}
        </ScrollArea>
