    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 text-slate-900 dark:text-slate-100 font-sans">
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-2 overflow-x-auto">
        ...
        >
          ...
          <span>EXTERNAL ACCOUNTS REGISTRY ({PUBLIC_ACCOUNT_REGISTRY.filter(a => a.status === 'ACTIVE').length} ACTIVE)</span>
        </button>
      </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
              <div>
                ...
                <h2 className="text-xl font-extrabold text-white mt-1">Growth OS External Account Registry</h2>
                <p className="text-xs text-slate-400">
                  Verified public accounts & discovery graph nodes for CHATR and Founder Sanobar Jahan. Zero credentials stored.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-slate-800 border border-slate-700 text-xs text-slate-300 rounded-lg font-mono">
                  Brand: CHATR
                </span>
                <a 
                  ...
                  className="px-3 py-1 bg-purple-600 hover:bg-purple-500 text-xs text-white font-bold rounded-lg transition-colors flex items-center gap-1"
                >
            ...
            <div className="grid md:grid-cols-2 gap-4 pt-2">
              <div className="bg-slate-950/80 border border-purple-500/20 rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-purple-400 flex items-center gap-1">
                    <UserCheck className="w-3.5 h-3.5" /> FOUNDER IDENTITY
                  </span>
                  <span className="text-[10px] bg-emerald-950 text-emerald-400 border border-emerald-800 px-2 py-0.5 rounded font-bold">
                    VERIFIED AUTHORITY NODE
                  </span>
                </div>
                <p className="text-sm font-extrabold text-white">{CANONICAL_FOUNDER_IDENTITY.name}</p>
                <p className="text-xs text-slate-400">{CANONICAL_FOUNDER_IDENTITY.positioning}</p>
                <div className="flex flex-wrap gap-2 pt-1">
                  <a href={CANONICAL_FOUNDER_IDENTITY.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 bg-blue-950/40 border border-blue-800/40 px-2.5 py-1 rounded hover:underline inline-flex items-center gap-1">
                    <ExternalLink className="w-3 h-3" /> LinkedIn (sanobarjahan12)
                  </a>
                  <a href={CANONICAL_FOUNDER_IDENTITY.redditUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-orange-400 bg-orange-950/40 border border-orange-800/40 px-2.5 py-1 rounded hover:underline inline-flex items-center gap-1">
                    <ExternalLink className="w-3 h-3" /> Reddit (u/SanobarJahan)
                  </a>
                </div>
              </div>

              <div className="bg-slate-950/80 border border-blue-500/20 rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                ...
                <p className="text-sm font-extrabold text-white">{CANONICAL_COMPANY_IDENTITY.brandName}</p>
                <p className="text-xs text-slate-400">{CANONICAL_COMPANY_IDENTITY.positioning}</p>
                <div className="flex flex-wrap gap-2 pt-1 text-xs">
                  <span className="text-emerald-400 bg-emerald-950/40 border border-emerald-800 px-2 py-0.5 rounded">Facebook CHATR Page</span>
                  <span className="text-emerald-400 bg-emerald-950/40 border border-emerald-800 px-2 py-0.5 rounded">Instagram @chatr_chat</span>
                  <span className="text-emerald-400 bg-emerald-950/40 border border-emerald-800 px-2 py-0.5 rounded">X @chatr_chat</span>
                  <span className="text-emerald-400 bg-emerald-950/40 border border-emerald-800 px-2 py-0.5 rounded">YouTube @chatr_chat</span>
                  <span className="text-rose-400 bg-rose-950/40 border border-rose-800 px-2 py-0.5 rounded">LinkedIn Page: Blocked (Needs connections)</span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-950 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800">
                  <tr>
                    ...
                    <th className="py-3 px-4">Public Handle / URL</th>
                    <th className="py-3 px-4">Purpose / Notes</th>
                    <th className="py-3 px-4">Verification</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60 font-sans">
                  {PUBLIC_ACCOUNT_REGISTRY.map(acc => (
                    <tr key={acc.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="py-3 px-4 font-bold text-white">
                        {acc.platform}
                      </td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${acc.account_type === 'founder' ? 'bg-purple-950 text-purple-300 border border-purple-800' : 'bg-blue-950 text-blue-300 border border-blue-800'}`}>
                          {acc.owner} ({acc.account_type})
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        {acc.status === 'ACTIVE' && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-950 text-emerald-400 border border-emerald-800">ACTIVE</span>}
                        {acc.status === 'PENDING' && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-950 text-amber-400 border border-amber-800">PENDING</span>}
                        {acc.status === 'BLOCKED' && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-950 text-rose-400 border border-rose-800">BLOCKED</span>}
                        {acc.status === 'NOT_CREATED' && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-400 border border-slate-700">NOT CREATED</span>}
                      </td>
                      <td className="py-3 px-4">
                        <span className="font-mono text-slate-300">{acc.username}</span>
                        {acc.public_url && acc.public_url.startsWith('http') && (
                          <a href={acc.public_url} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:underline block text-[11px] mt-0.5 flex items-center gap-1">
                            Visit <ExternalLink className="w-3 h-3" />
                          </a>
                        )}
                      </td>
                      <td className="py-3 px-4 max-w-xs space-y-1">
                        <p className="text-slate-300">{acc.purpose}</p>
                        {acc.notes && (
                          <p className="text-[10px] text-rose-300 bg-rose-950/40 p-1.5 rounded border border-rose-900">
                            ⚠️ {acc.notes}
                          </p>
                        )}
                      </td>
